...
/* [WLAN][SHARP] 2012.06.15 Customized for debug Start */
#if 0
#define WL_TRACE(x)
#define WL_ASSOC(x)
#define WL_INFORM(x)
...
#define WL_SCAN(x)
#else
#define WL_DBG_TRACE	0x00100000
#define WL_DBG_ASSOC	0x00080000
#define WL_DBG_INFORM	0x00040000
#define WL_DBG_WSEC		0x00020000
#define WL_DBG_SCAN		0x00010000

extern int dhd_msg_level;
#define	WL_TRACE(args)	do { if (dhd_msg_level & WL_DBG_TRACE)	{ printk(KERN_ERR "WL-TRACE) %s :"	, __func__); printk args; } } while (0)
#define	WL_ASSOC(args)	do { if (dhd_msg_level & WL_DBG_ASSOC)	{ printk(KERN_ERR "WL-ASSOC) %s :"	, __func__); printk args; } } while (0)
#define	WL_INFORM(args)	do { if (dhd_msg_level & WL_DBG_INFORM)	{ printk(KERN_ERR "WL-INFORM) %s :"	, __func__); printk args; } } while (0)
#define	WL_WSEC(args)	do { if (dhd_msg_level & WL_DBG_WSEC)	{ printk(KERN_ERR "WL-WSEC) %s :"	, __func__); printk args; } } while (0)
#define	WL_SCAN(args)	do { if (dhd_msg_level & WL_DBG_SCAN)	{ printk(KERN_ERR "WL-SCAN) %s :"	, __func__); printk args; } } while (0)
#endif
/* [WLAN][SHARP] 2012.06.15 Customized for debug End */
...
/* [WLAN][SHARP] 2011.03.31 Remedy for BT Co-ex problem Start */
#if 0
#define BT_DHCP_eSCO_FIX 
#define BT_DHCP_USE_FLAGS  
#else
#undef BT_DHCP_eSCO_FIX
#undef BT_DHCP_USE_FLAGS
#endif
/* [WLAN][SHARP] 2011.03.31 Remedy for BT Co-ex problem End */
...
)
{
	...
	int band;
	int chan = 1;
/* [WLAN][SHARP] 2011/03/03 <= RegulatoryDomain */
...
#endif 
/* [WLAN][SHARP] 2011.04.11 Add POWERSAVE add Start */
static int
wl_iw_set_power_save(
	struct net_device *dev,
	struct iw_request_info *info,
	union iwreq_data *wrqu,
	char *extra
)
{
	...
	WL_TRACE_COEX(("%s: powersave cmd:%s\n", __FUNCTION__, extra));

	strncpy((char *)&powermode_val, extra + strlen("POWERSAVE") +1, 1);
...
/* [WLAN][SHARP] 2011.04.11 Add POWERSAVE add End */

/* [WLAN][SHARP] Customize start
									2012.02.13 Add wifi eco mode 
									2012.07.06 disabled for beacon interval problem */
#define SH_BCN_LI_DTIM 5    /* skip 4 DTIM */
int wl_iw_set_eco(
	struct net_device *dev,
	struct iw_request_info *info,
	union iwreq_data *wrqu,
...
)
{
	...
	WL_TRACE_COEX(("%s: set eco cmd:%s\n", __FUNCTION__, extra));

	strncpy((char *)&eco_val, extra + strlen("SETECO") +1, 1);

	if (strnicmp((char *)&eco_val, "1", strlen("1")) == 0) {
		WL_TRACE_COEX(("%s: enable wifi eco mode\n", __FUNCTION__));
		net_os_set_dtim_skip(dev, SH_BCN_LI_DTIM);
	} else {
		WL_TRACE_COEX(("%s: disable wifi eco mode\n", __FUNCTION__));
...
)
{
	...
	if (g_onoff == G_WLAN_SET_ON) {
		...
		/* [WLAN][SHARP] 2012/06/18 <= : RSSI compensation for BCM4329, total loss: 9dBm */
		WL_TRACE(("%s actual rssi :%d\n",	__FUNCTION__, rssi ));
		rssi += 9;
		/* [WLAN][SHARP] 2012/06/18 => : RSSI compensation */
	...
	case IW_AUTH_WPA_ENABLED:
		if (paramval == 0) {
			...
			WL_INFORM(("%s: %d: setting wpa_auth to %d\n",
				__FUNCTION__, __LINE__, val));
/* [WLAN][SHARP] 2011.04.25 Delete unnecessary Log */
#if 0
			error = dev_wlc_intvar_set(dev, "wpa_auth", 0);
#else
			if ((error = dev_wlc_intvar_set(dev, "wpa_auth", 0)))
#endif
/* [WLAN][SHARP] 2011.04.25 Delete unnecessary Log */
			WL_ERROR(("Failed to set 'wpa_auth'iovar\n"));
			return error;
...
/* [WLAN][SHARP] 2011.04.11 Add POWERSAVE add Start */
		...
		{
			ret = wl_iw_set_power_save(dev, info, (union iwreq_data *)dwrq, extra);
		}
/* [WLAN][SHARP] 2011.04.11 Add POWERSAVE add End */
/* [WLAN][SHARP] 2012.02.13 Add wifi eco mode Start */
	    else if (strnicmp(extra, "SETECO", strlen("SETECO")) == 0)
		{
			ret = wl_iw_set_eco(dev, info, (union iwreq_data *)dwrq, extra);
		}
/* [WLAN][SHARP] 2012.02.13 Add wifi eco mode End */
#ifdef SOFTAP
...
/* [WLAN][SHARP] 2011.04.14 Add Connected BSSID clear Start */
			memset(g_ss_cache_ctrl.m_active_bssid, 0, ETHER_ADDR_LEN);
/* [WLAN][SHARP] 2011.04.14 Add Connected BSSID clear End */
