 ...
 * ---------------------------------------------------------------------------
 *  fw_diag_check
 *      Start Mode Diag or Normal Check
 *
 *  Arguments: 
...
/*[WLAN][SHARP] Customize Start
                2010.01.20 Mod Start
                2012.05.17 Mod for ICS */
#if defined( CONFIG_SHWLAN_BCM4329_1 )
int fw_diag_check(void)
{
    struct file *file;
    char *filename = "/sys/devices/platform/wifipm/shdiag";
    int ibuf=0;
    char cbuf[2] = {'\0','\0'};     /* Initialize */
    ...
    file->f_op->read(file, cbuf, 1, &(file->f_pos));

    sscanf( &cbuf[0], "%1d", &ibuf );
...
/* [WLAN][SHARP] 2011.08.29 Initialization error ap_fw_loaded Start */
	ap_fw_loaded = FALSE;
/* [WLAN][SHARP] 2011.08.29 Initialization error ap_fw_loaded End */
#ifdef CONFIG_BCM4329_DIAG_FW_PATH
	if ( fw_diag_check() == 1 )
		/* Diag Mode */
		strncpy(fw_path, CONFIG_BCM4329_DIAG_FW_PATH, MOD_PARAM_PATHLEN-1);
	else
#endif
...
#endif /* defined( CONFIG_SHWLAN_BCM4329_1 ) && !defined(SH_ENABLE_INSMOD_NO_FW_LOAD) */
...
/* [WLAN][SHARP] 2012.05.17 bcn_timeout 10 -> 3 Start */
#if 0
	uint bcn_timeout = 4;
#else
	uint bcn_timeout = 3;
#endif
/* [WLAN][SHARP] 2012.05.17 bcn_timeout 10 -> 3 End */

/* [WLAN][SHARP] 2011.02.21 customize scan time Start */
#if 0
	int scan_assoc_time = 40;
	int scan_unassoc_time = 40;
	int scan_passive_time = 130;
#else
	int scan_assoc_time = 30;
	int scan_unassoc_time = 30;
    /* [WLAN][SHARP] 2011.03.11 for consumption power decrease.210->120 */
	int scan_passive_time = 120;
    /* [WLAN][SHARP] 2011.03.11 for prevention of ProbeRequest lost.1->2 */
	int scan_nprobes = 2;         
#endif
/* [WLAN][SHARP] 2011.02.21 customize scan time End */

/* [WLAN][SHARP] 2011.02.21 enable short guard interval Start */
#if 0
	uint sgi_rx = 1;
	uint sgi_tx = 1;
#endif
/* [WLAN][SHARP] 2011.02.21 enable short guard interval End */
...
/* [WLAN][SHARP] 2011/03/03 <= : Broadcom CASE #? RegulatoryDomain */

        if (ap_fw_loaded == FALSE) {

/* [WLAN][SHARP] 2011.04.14 Remedy for RF power level Start */
#if 0
                wl_country_t xy = {"XY", 4, "XY"};
#else
                wl_country_t xy = {"XY", 3, "XY"};
#endif
/* [WLAN][SHARP] 2011.04.14 Remedy for RF power level End */



                bcm_mkiovar("country", (char *)&xy, sizeof(wl_country_t),
iovbuf, sizeof(iovbuf));
                if ((ret = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf,
sizeof(iovbuf))) < 0)
                        DHD_ERROR(("%s: country code setting failed\n",
__FUNCTION__));
        }
...
/* [WLAN][SHARP] 2011.02.26 customise scan behavior Start */
#if 0
	...
	dhdcdc_set_ioctl(dhd, 0, WLC_SET_SCAN_PASSIVE_TIME, (char *)&scan_passive_time,
		sizeof(scan_unassoc_time));
	dhdcdc_set_ioctl(dhd, 0, WLC_SET_SCAN_NPROBES, (char *)&scan_nprobes,
		sizeof(scan_unassoc_time));
#endif
/* [WLAN][SHARP] 2011.02.21 customise scan behavior End */
...
int dhd_get_dtim_skip(dhd_pub_t *dhd)
{
	...
	int ret;
	int dtim_assoc = 0;
    #if 0   /* [WLAN][SHARP] 2011.03.16 disable dtim skipping Start */
	if ((dhd->dtim_skip == 0) || (dhd->dtim_skip == 1))
		bcn_li_dtim = 3;
	else
		bcn_li_dtim = dhd->dtim_skip;
    #else
    /* We do not need to dtim skipping */
	bcn_li_dtim = 0;
    #endif  /* [WLAN][SHARP] 2011.03.16 disable dtim skipping End */
