...
struct msm_sensor_ctrl {
	...
	int (*s_otp_read)(unsigned char page, unsigned char offset,
						short len, unsigned char* buf);
	int (*s_power_ctrl)(struct platform_device *pdev, int8_t power);
	int (*s_set_af_table)(unsigned short start,
						unsigned short end, unsigned short step);
	int (*s_reset_af_table)(void);
#elif defined(CONFIG_S5K4EC)
	int (*s_camcon)(void);
	int (*s_reg_read)(unsigned short wpage,
					unsigned short waddr, unsigned short* wdata);
	int (*s_reg_write)(unsigned short wpage,
					unsigned short waddr, unsigned short wdata);
	int (*s_otp_read)(unsigned char page, unsigned char offset,
						short len, unsigned char* buf);
	int (*s_power_ctrl)(struct platform_device *pdev, int8_t power);
