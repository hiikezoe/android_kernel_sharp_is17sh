...
#elif ( CONFIG_SH_AUDIO_DRIVER_MODEL_NUMBER == 305 )
	GPIO_CFG(138, 1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_10MA),
	GPIO_CFG(139, 1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(140, 1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_10MA),
	GPIO_CFG(141, 1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_10MA),
#else
...
static void aux_pcm_gpios_disable(void)
{
	int pin, rc;
...
#ifdef CONFIG_SH_AUDIO_DRIVER
	rc = aux_pcm_gpios_enable();
	if (rc) {
		MM_ERR("GPIO enable for AUX PCM failed\n");
		return rc;
	}
#endif /* CONFIG_SH_AUDIO_DRIVER */
...
#ifdef CONFIG_SH_AUDIO_DRIVER
static int aux_pcm_suspend(struct platform_device *pdev, pm_message_t state)
{
#ifdef CONFIG_SHSYS_CUST
