...
#ifdef CONFIG_SH_AUDIO_DRIVER
int msm_set_device_mute(int dir, int mute)
{
	MM_DBG("dir %x mute %x\n", dir, mute);
	if (dir == DIR_RX) {
		if(mute == true){
			MM_DBG("Mute");
			afe_device_volume_ctrl(AFE_HW_PATH_CODEC_RX, 0x0000);
			afe_device_volume_ctrl(AFE_HW_PATH_AUXPCM_RX, 0x0000);
		}else{
			MM_DBG("UnMute");
			afe_device_volume_ctrl(AFE_HW_PATH_CODEC_RX, 0x4000);
			afe_device_volume_ctrl(AFE_HW_PATH_AUXPCM_RX, 0x4000);
		}
	}
		
	return 0;
}
EXPORT_SYMBOL(msm_set_device_mute);
#endif /* CONFIG_SH_AUDIO_DRIVER */
		...
		if (callback->clnt_type == AUDDEV_CLNT_VOC) {
			...
			if (evt_id == AUDDEV_EVT_DEVICE_VOL_MUTE_CHG) {
				if (dev_info->capability & SNDDEV_CAP_TX) {
					...
					MM_INFO("mute=%d rx_vol=%d\n", evt_payload->voc_vm_info.rx_mute, evt_payload->voc_vm_info.dev_vm_val.vol);
#endif /* CONFIG_SH_AUDIO_DRIVER */
				}
				...
				if (dev_info->capability & SNDDEV_CAP_RX) {
					...
					for (i = 0; i < VOC_RX_VOL_ARRAY_NUM;i++) {
						memcpy(evt_payload->voc_devinfo.rx_vol_step[i],
								dev_info->voc_rx_vol_step[i],
								sizeof(evt_payload->voc_devinfo.rx_vol_step[i])
							);
					}
#else
