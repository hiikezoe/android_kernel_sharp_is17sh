...
static int audio_lpa_status = 0;

static void set_audio_status_enable(void){
	MM_DBG("%d \n", audio_lpa_status); /* Macro prints the file name and function */
	
	if(!audio_lpa_status){
		MM_DBG("set music status 1\n");
		shsd_set_music_status(1);
		audio_lpa_status = 1;
	}
}

static void set_audio_status_disable(void){
	MM_DBG("%d\n", audio_lpa_status); /* Macro prints the file name and function */
	
	if(audio_lpa_status){
		MM_DBG("set music status 0\n");
		shsd_set_music_status(0);
		audio_lpa_status = 0;
	}
}
...
#ifdef CONFIG_SH_AUDIO_DRIVER
					{
						struct audpp_cmd_cfg_object_params_volume _vol_pan;
						memcpy(&_vol_pan, &audio->vol_pan, sizeof(struct audpp_cmd_cfg_object_params_volume));
						_vol_pan.volume = 0x2000;
						audpp_dsp_set_vol_pan(
							audio->dec_id,
							&_vol_pan, POPP);
					}
...
#ifdef CONFIG_SH_AUDIO_DRIVER
		{
			long old_vol = audio->vol_pan.volume;
			long new_vol = evt_payload->session_vol;
			int i;
			MM_DBG("\n:AUDDEV_EVT_STREAM_VOL_CHG, stream vol (old:%d, new:%d)\n"
				"running = %d, enabled = %d, source = 0x%x",
				audio->vol_pan.volume, evt_payload->session_vol,
				audio->running, audio->enabled, audio->source);
			if (audio->running == 1 && audio->enabled == 1) {
				for(i = 1; i <= 35; i++) {
					audio->vol_pan.volume = (unsigned short)((((new_vol - old_vol) * i) / 35) + old_vol);
					MM_DBG("--- [%d] vol=%d ---\n", i, audio->vol_pan.volume);
					if (audio->source & AUDPP_MIXER_HLB)
						audpp_dsp_set_vol_pan(
							AUDPP_CMD_CFG_DEV_MIXER_ID_4,
							&audio->vol_pan, COPP);
