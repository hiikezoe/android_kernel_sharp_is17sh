...
#if defined(CONFIG_USB_ANDROID_SH_CUST)
	unsigned long	set_qxdmen;
	unsigned long	get_qxdmen;
#endif	/* defined(CONFIG_USB_ANDROID_SH_CUST) */
};
...
#if defined(CONFIG_USB_ANDROID_SH_CUST)
		usb_rpc_ids.set_qxdmen			= 30;
		usb_rpc_ids.get_qxdmen			= 31;
#endif	/* defined(CONFIG_USB_ANDROID_SH_CUST) */
		return 0;
...
static int msm_chg_init_rpc(unsigned long vers)
{
	...
	if (((vers & RPC_VERSION_MAJOR_MASK) == 0x00010000) ||
		...
		chg_ep = msm_rpc_connect_compatible(SHCHG_REMOTE_A2MPROG, vers,
...
int msm_hsusb_set_qxdmen(uint32_t enable)
{
	...
	if (rc < 0) {
		pr_err("%s: set_qxdmen failed! rc = %d\n",
			__func__, rc);
		return rc;
	}

	pr_info("%s:result (%d)\n", __func__, be32_to_cpu(rep.result));
...
#else	/* defined(CONFIG_USB_ANDROID_SH_CUST) */
	msm_chg_usb_i_is_available(mA);
#endif	/* defined(CONFIG_USB_ANDROID_SH_CUST) */
}
