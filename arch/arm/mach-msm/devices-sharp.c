...
static struct resource mddi_sharp_qvga_resources[] = {
	{
		.name   = "shlcdc",
		.start  = 0x8c000000,
		.end    = 0x8c000000 + 0x1000 - 1,
		.flags  = IORESOURCE_MEM,
	}
};

static struct platform_device mddi_sharp_qvga_device = {
	.name   = "mddi_sharp_qvga",
	.id     = 0,
	.num_resources  = ARRAY_SIZE(mddi_sharp_qvga_resources),
	.resource       = mddi_sharp_qvga_resources,
};
...
#endif
#ifdef CONFIG_SHLCDC_BOARD /* LCD add */
	else if (!strncmp(name, "shlcdc", 6))
		msm_register_device(&mddi_sharp_qvga_device, data);
#endif	/* CONFIG_SHLCDC_BOARD */
	else
