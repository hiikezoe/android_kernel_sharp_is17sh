...
#if 1
    #define CFG_GET_AF_LENS_POSITION	36
    #define CFG_SET_AF_DEFAULT_CODE		37
    #define CFG_SET_FOCUS_MODE			38
    #define CFG_SET_AF_TABLE			39
    #define CFG_SET_DIMENSION			40
    #define CFG_GET_AF_SEARCH_STATUS	41
    #define CFG_SET_FOCUS_AREA			42
    #define CFG_SET_ISO					43
    #define CFG_SET_SCENE				44
    #define CFG_GET_SNAPSHOT_INFO		45
    #define CFG_GET_SENSOR_STAT			46
    #define CFG_SET_CANCEL_AUTO_FOCUS	47
    #define CFG_SET_LOCK_UNLOCK			48
    #define CFG_SET_GAMMA				49
    #define CFG_GET_CAMERA_INFO			50
    #define CFG_MAX						51
#else
    #define CFG_MAX			36
#endif
...
#if 1
    #define CAMERA_FOCUS_NORMAL		0
    #define CAMERA_FOCUS_MACRO		1
    #define CAMERA_FOCUS_AUTO		2
    #define CAMERA_FOCUS_CAF		3
    #define CAMERA_FOCUS_PRODUCT	4
    #define CAMERA_FOCUS_MAX		5

    #define CAMERA_SCENE_OFF			0
    #define CAMERA_SCENE_POTRAIT		1
    #define CAMERA_SCENE_POTRAITNIGHT	2
    #define CAMERA_SCENE_LANDSCAPE		3
    #define CAMERA_SCENE_NIGHTSHOT		4
    #define CAMERA_SCENE_FOOD			5
    #define CAMERA_SCENE_TEXT			6
    #define CAMERA_SCENE_SPORTS			10

    #define CAMERA_LOCK_UNLOCK_AEC		1
    #define CAMERA_LOCK_UNLOCK_AWB		2

    #define CAMERA_GAMMA_WIDE			0
    #define CAMERA_GAMMA_LIMITED		1
#endif
...
struct focus_default_cfg {
	uint16_t normal_default_position;
	uint16_t macro_default_position;
	uint16_t full_default_position;
	uint16_t product_default_position;
};

struct focus_table_cfg {
	int16_t full_step;
	int16_t full_start;
	int16_t full_stop;
	int16_t macro_step;
...
struct input_request {
	struct input_set ins;
	struct input_set zms;
};

struct dimension_cfg {
	int16_t preview_width;
	int16_t preview_height;
	int16_t capture_width;
	int16_t capture_height;
	struct input_request pre_input;
	struct input_request cap_input;
};
...
struct lock_unlock_cfg {
	int lock_type;
	int lock_unlock;
};

struct shcam_event_ctrl_cmd {
 unsigned int Id;
 unsigned int Param1;
 unsigned int Param2;
 unsigned int Param3;
...
struct shcam_production_info_cmd{
 unsigned long mode;
 void *value;
};

struct shcam_ctrl_cmd {
 unsigned char r_data[0x80];
 unsigned char w_data[0x80];
 signed int status;
 signed int length;
 unsigned short addr;
 ...
 struct shcam_event_ctrl_cmd event_data;
 struct shcam_production_info_cmd product_data;
 unsigned char* pData;
};
...
struct sensor_snapshot_info_cfg {
	uint32_t exp_value;
	uint16_t a_gain;
	uint16_t d_gain;
};

struct sensor_get_stat_cfg {
	void *pstat;
	int statsize;
};

struct camera_info_cfg {
 uint16_t lens_pos;
 uint32_t bv;
};
#endif
...
#if 1
		struct sensor_wb_calib_data wb_calib_info;
		struct focus_default_cfg focus_default_info;
		uint8_t focus_mode;
		uint8_t iso_val;
		uint8_t scene_mode;
		uint8_t gamma;
		struct focus_table_cfg focus_table_info;
		struct dimension_cfg dimension_info;
		struct zoom_cfg zoom_info;
		struct lock_unlock_cfg lock_unlock_info;
		struct shcam_ctrl_cmd sh_ctrl_data;
		struct focus_search_info_cfg focus_info;
		struct focus_area_info_cfg focus_area_info;
		struct sensor_snapshot_info_cfg sensor_snapshot_info;
		struct sensor_get_stat_cfg sensor_get_stat;
		struct camera_info_cfg camera_info;
#endif
	} cfg;
};
