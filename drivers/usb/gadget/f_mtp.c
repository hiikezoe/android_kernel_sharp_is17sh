...
#define VENDOR_CODE_MTP 				0x08
#define MTP_EXT_CONFIG_DESC_FUNC_NUM	4
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
	struct mtp_ext_config_desc_function    function[MTP_EXT_CONFIG_DESC_FUNC_NUM];
#else /* CONFIG_USB_ANDROID_SH_CUST */
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
		if (ctrl->bRequest == VENDOR_CODE_MTP
#else /* CONFIG_USB_ANDROID_SH_CUST */
		if (ctrl->bRequest == 1
#endif /* CONFIG_USB_ANDROID_SH_CUST */
			...
			if (config) {
				int interfaceNum;
				int mtp_ext_config_desc_size;

				interfaceNum = config->next_interface_id;
				if (interfaceNum > MTP_EXT_CONFIG_DESC_FUNC_NUM) {
					ERROR(cdev, "%s:interfaceNum over %d\n",
						__func__, MTP_EXT_CONFIG_DESC_FUNC_NUM);
					interfaceNum = MTP_EXT_CONFIG_DESC_FUNC_NUM;
				}
				mtp_ext_config_desc_size = sizeof(struct mtp_ext_config_desc_header) + 
						(interfaceNum * sizeof(struct mtp_ext_config_desc_function));

				mtp_ext_config_desc.header.dwLength = __constant_cpu_to_le32(mtp_ext_config_desc_size);
				mtp_ext_config_desc.header.bCount = __constant_cpu_to_le32(interfaceNum);

				value = (w_length < mtp_ext_config_desc_size ?
						w_length : mtp_ext_config_desc_size);

				value = value > cdev->bufsiz ? cdev->bufsiz : value;
