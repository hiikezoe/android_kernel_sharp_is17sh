	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	printk("[lismo_buf_cntrol]%s desc->len=%08x ALLOC_INI_SIZE=%08x \n",__func__, desc->len, ALLOC_INI_SIZE);
	...
	if (!test_bit(FLAG_EXPORT, &desc->flags)) {
		...
		if ( cmd-SC_VENDOR_START+1 > ALLOC_CMD_CNT || desc->len != ALLOC_INI_SIZE){
			printk("[lismo_buf_cntrol]%s free old buffer \n",__func__);
			kfree(desc->buffer);
		}
		desc->len = len;
		desc->buffer = buffer;
...
static int vendor_cmd_export(struct device *dev, unsigned cmd, int init)
{
	...
	printk("[lismo_buf_cntrol]%s start\n",__func__);
	...
	status = 0;
	if (test_bit(FLAG_EXPORT, &desc->flags)) {
		printk("[lismo_buf_cntrol]%s id:%02x alrdy created\n",__func__,cmd);
		goto done;
	}

	if ( cmd-SC_VENDOR_START+1 > ALLOC_CMD_CNT ){
		desc->buffer = kzalloc(2048, GFP_KERNEL);
		printk("[lismo_buf_cntrol]%s opcode:%02x bufalloc size:%08x \n", __func__, cmd, 2048);
		if(!desc->buffer) {
			status = -ENOMEM;
			goto done;
		}
		...
		printk("[lismo_buf_cntrol]%s opcode:%02x bufcopy bufsize:%08x \n", __func__, cmd, ALLOC_INI_SIZE);
	...
	if ( cmd-SC_VENDOR_START+1 > ALLOC_CMD_CNT ){
		printk("%s opcode:%02x bufsize:%08x \n", __func__, cmd, 2048);
	...
	desc->update = 0;
	printk("[lismo_buf_cntrol]%s opcode:%02x success\n",__func__,cmd);
...
static int lun_regist(struct platform_device *pdev)
{
	...
	printk("[lismo_buf_cntrol]%s pdev.name:%s\n",__func__,dev_name(&pdev->dev));
#if 0
...
static int __init init(void)
{
	...
	printk("[lismo_buf_cntrol]%s platform_driver_probe rc:%08x\n", __func__, rc);
	return rc;
}
