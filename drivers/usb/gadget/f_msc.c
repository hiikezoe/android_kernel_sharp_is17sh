...
#define USB_MSC_FUNC_IOC_GET_MOUNT_STS _IOR(USB_MSC_IOC_MAGIC, 0x21, int)
#define USB_MSC_FUNC_IOC_GET_VENDOR_ID _IOR(USB_MSC_IOC_MAGIC, 0x22, int)
#define USB_MSC_FUNC_IOC_GET_PRODUCT_ID _IOR(USB_MSC_IOC_MAGIC, 0x23, int)
#ifdef D_MSC_STORE_MOUNT_FILE_PATH
...
static long msc_ioctl(struct file *file, unsigned int code, unsigned long arg)
{
...
static ssize_t show_msc_file(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	ssize_t		rc;
#ifndef D_MSC_STORE_MOUNT_FILE_PATH
...
static struct device_attribute dev_attr_msc_file = __ATTR(file, 0644, show_msc_file, store_msc_file);
...
static int msc_bind_config(struct usb_configuration *c)
{
	...
	printk(KERN_INFO "msc_bind_config\n");
...
static struct msc_dev* msc_setup(struct usb_composite_dev *cdev)
{
	struct msc_dev *dev;
	int ret;
	struct lun	*curlun; /* MOUNT_FILE */
	...
	if (curlun) {
		...
		if (ret != 0) {
			printk(KERN_ERR "%s:device_register(lun1) failed\n", __func__);
			goto err2;
		}
		ret = device_create_file(&curlun->dev, &dev_attr_msc_file);
		if (ret != 0) {
			printk(KERN_ERR "%s:device_create_file(lun1) failed\n", __func__);
			device_unregister(&curlun->dev);
			goto err3;
		}
	}
	/* default string set */
	memcpy(_msc_dev->msc_inquiry_string, USB_MSC_INQUIRY_STRING, USB_MSC_INQUIRY_STRING_LEN);
	return dev;
