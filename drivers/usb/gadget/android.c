...
#endif /* CONFIG_USB_ANDROID_SH_CUST */
#if !defined(CONFIG_USB_ANDROID_SH_CUST) || defined(CONFIG_USB_ANDROID_SH_SERIALS)
#include "f_serial.c"
#include "f_acm.c"
#endif /* CONFIG_USB_ANDROID_SH_CUST && CONFIG_USB_ANDROID_SH_SERIALS */
#ifdef CONFIG_USB_ANDROID_SH_SERIALS
#include "f_obex.c"
#include "f_sh_serial.c"
...
#define D_USB_DISCHARGE_MAX_POWER		(100/2)
#endif /* CONFIG_USB_ANDROID_SH_CUST */
...
static ssize_t diag_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct android_usb_function *f = dev_get_drvdata(dev);
	struct diag_function_config *diag;
	...
	return snprintf(buf, PAGE_SIZE, "%d", diag->enable);
}

static ssize_t diag_enable_store(
		...
		const char *buf, size_t size)
{
	...
	if (!buf || size <= 0 || size > 2) {
		pr_err("%s: size = %d\n", __func__, size);
		return -EOPNOTSUPP;
	}

	if ((size == 2) && (*(buf+1) != 0x0a) && (*(buf+1) != 0x00)) {
		pr_err("%s: size = %d, buf = %c\n", __func__, size, *buf);
		return -EOPNOTSUPP;
	}
					...
					struct usb_configuration *c)
{
	...
	if(!diag->enable) {
		pr_debug("[%s]diag disable", __func__);
		return 0;
	}
#endif /* CONFIG_USB_ANDROID_SH_CUST */
...
{
	return sh_serial_bind_config(c, f->name);
}

static struct device_attribute *sh_obex_function_attributes[] = {
...
static struct android_usb_function obex_function = {
	...
	.attributes		= sh_obex_function_attributes,
};
...
static int sh_mdlm_function_init(struct android_usb_function *f, struct usb_composite_dev *cdev)
{
	return sh_serial_setup(cdev);
}

static void sh_mdlm_function_cleanup(struct android_usb_function *f)
{
	sh_serial_cleanup();
}

static int sh_mdlm_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	set_guid_value(mdlm_guid);
	return sh_serial_bind_config(c, f->name);
}

static ssize_t mdlm_guid_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
...
static ssize_t mdlm_guid_store(
		struct device *device, struct device_attribute *attr,
		const char *buff, size_t size)
{
	int len;
...
static ssize_t mdlm_multiple_open_show(struct device *pdev,
					 struct device_attribute *attr,
					 char *buf)
{
	int value;

	value = snprintf(buf, PAGE_SIZE, "%d", mdlm_multiple_open);

	return value;
}

static ssize_t mdlm_multiple_open_store(struct device *pdev,
						struct device_attribute *attr,
						const char *buf, size_t size)
{
	int allow;
	...
	ret = sscanf(buf, "%d", &allow);
	if (ret == 1) {
		if (allow)
			mdlm_multiple_open = true;
		else
			mdlm_multiple_open = false;
	} else {
		pr_err("%s: invalid input\n", __func__);
	}
...
static DEVICE_ATTR(multiple_open, S_IRUGO | S_IWUSR,
					mdlm_multiple_open_show, mdlm_multiple_open_store);

static struct device_attribute *sh_mdlm_function_attributes[] = {
	/* define the device attributes in sh_string.c */
	&dev_attr_mdlm_iInterface,
	&dev_attr_guid,
	&dev_attr_multiple_open,
	NULL
};

static struct android_usb_function mdlm_function = {
	.name			= "mdlm",
	.init			= sh_mdlm_function_init,
	.cleanup		= sh_mdlm_function_cleanup,
	.bind_config	= sh_mdlm_function_bind_config,
	.attributes		= sh_mdlm_function_attributes,
};

/* MODEM */
static int sh_modem_function_init(struct android_usb_function *f, struct usb_composite_dev *cdev)
{
	return sh_serial_setup(cdev);
}

static void sh_modem_function_cleanup(struct android_usb_function *f)
{
	sh_serial_cleanup();
}

static int sh_modem_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	return sh_serial_bind_config(c, f->name);
}

static struct device_attribute *sh_modem_function_attributes[] = {
	/* define the device attributes in sh_string.c */
	&dev_attr_mdm_iInterface,
	NULL
};

static struct android_usb_function modem_function = {
	.name			= "modem",
	.init			= sh_modem_function_init,
	.cleanup		= sh_modem_function_cleanup,
	.bind_config	= sh_modem_function_bind_config,
	.attributes		= sh_modem_function_attributes,
};
#endif /* CONFIG_USB_ANDROID_SH_SERIALS */
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	struct rndis_function_config *rndis = f->config;
#ifdef CONFIG_USB_ANDROID_SH_CUST
	int ethaddr[ETH_ALEN];

	if (sscanf(buf, "%02x:%02x:%02x:%02x:%02x:%02x\n",
			&ethaddr[0], &ethaddr[1],
			&ethaddr[2], &ethaddr[3],
			&ethaddr[4], &ethaddr[5]) == 6) {
        int i;
		for(i=0; i<ETH_ALEN; i++)
			rndis->ethaddr[i] = (u8)(ethaddr[i] & 0xFF);
		return size;
	}
#else /* CONFIG_USB_ANDROID_SH_CUST */
	if (sscanf(buf, "%02x:%02x:%02x:%02x:%02x:%02x\n",
...
static DEVICE_ATTR(msc_inquiry_string, S_IRUGO | S_IWUSR,
					msc_inquiry_show,
					msc_inquiry_store);
	...
	if (enabled && !dev->enabled) {
		...
		if (dev->selfPower) {
			if (!(android_config_driver.bmAttributes & USB_CONFIG_ATT_SELFPOWER)) {
				...
				android_config_driver.bMaxPower = D_USB_DISCHARGE_MAX_POWER;
				ret = usb_gadget_set_selfpowered(cdev->gadget);
			}
		} else {
			if (android_config_driver.bmAttributes & USB_CONFIG_ATT_SELFPOWER) {
				android_config_driver.bmAttributes &= ~USB_CONFIG_ATT_SELFPOWER;
				android_config_driver.bMaxPower = D_USB_CHARGE_MAX_POWER;
				ret = usb_gadget_clear_selfpowered(cdev->gadget);
			}
		}
		if (ret < 0)
			printk(KERN_ERR "%s: selfPower cant set(%d) \n", __func__, ret);

		if (dev->forceFullSpeed_flg) {
			if (dev->forceFullSpeed)
				ret = usb_gadget_force_fullspeed(cdev->gadget);
			else
				ret = usb_gadget_allow_highspeed(cdev->gadget);
			if (ret < 0)
				printk(KERN_ERR "%s: forceFullSpeed cant set(%d) \n", __func__, ret);

			/* Wait for preparation of the host (windows xp). */
			msleep(500);
			dev->forceFullSpeed_flg = 0;
		}
#endif /* CONFIG_USB_ANDROID_SH_CUST */
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
static ssize_t selfPower_show(struct device *pdev,
					 struct device_attribute *attr,
					 char *buf)
{
	...
	value = snprintf(buf, PAGE_SIZE, "%d", adev->selfPower);

	return value;
}

static ssize_t selfPower_store(struct device *pdev,
						struct device_attribute *attr,
						const char *buf, size_t size)
{
	struct android_dev *adev = dev_get_drvdata(pdev);
...
static ssize_t forceFullSpeed_show(struct device *pdev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct android_dev *adev = dev_get_drvdata(pdev);
	...
	value = snprintf(buf, PAGE_SIZE, "%d", adev->forceFullSpeed);

	return value;
}

static ssize_t forceFullSpeed_store(struct device *pdev,
						struct device_attribute *attr,
						const char *buf, size_t size)
{
	struct android_dev *adev = dev_get_drvdata(pdev);
	...
	ret = sscanf(buf, "%d", &enable);
	if (ret == 1) {
		if ((adev->forceFullSpeed && !enable) || (!adev->forceFullSpeed && enable))
			adev->forceFullSpeed_flg = 1;
		if (enable)
			adev->forceFullSpeed = 1;
		else
			adev->forceFullSpeed = 0;
	} else {
		pr_err("%s: invalid input\n", __func__);
	}
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
static DEVICE_ATTR(selfPower, S_IRUGO | S_IWUSR, selfPower_show, selfPower_store);
static DEVICE_ATTR(forceFullSpeed, S_IRUGO | S_IWUSR, forceFullSpeed_show, forceFullSpeed_store);
