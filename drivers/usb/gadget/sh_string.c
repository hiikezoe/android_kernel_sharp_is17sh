...
static char mdm_iInterface[USB_I_INT_STRING_DESC_LEN];
...
{										\
	char *line_feed;					\
	if (size > len) pr_err("%s:over input length\n", __func__);		\
...
DESCRIPTOR_STRING_ATTR(mdm_iInterface, mdm_iInterface, USB_I_INT_STRING_DESC_LEN)
#endif /* CONFIG_USB_ANDROID_SH_SERIALS */
#ifdef CONFIG_USB_ANDROID_SH_UMS
