	...
	size_t count, loff_t *pos)
{
	...
	struct usb_request *req;
#ifdef CONFIG_USB_ANDROID_SH_CUST
	int r = count, xfer = 0, send_data_size = 0;
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
	/* we acquire data only to count size, when count size is smaller than max packet size. */
#endif /* CONFIG_USB_ANDROID_SH_CUST */
...
requeue_req:
	...
	ret = wait_event_interruptible(dev->read_wq, dev->rx_done);
			...
			ret = wait_event_interruptible_timeout(dev->read_wq, dev->rx_done, HZ / 100);
			if(ret == 0 && dev->rx_done == 0) {
				r = xfer;
				goto done;
			}
		}
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
		xfer += req->actual;
		send_data_size = req->actual;
		if(count <= xfer) {
			send_data_size -= (xfer - count);
			xfer = count;
		}
		pr_debug("rx %p %d\n", req, send_data_size);
		if (copy_to_user(buf, req->buf, send_data_size)) {
			r = -EFAULT;
			goto done;
		}
	...
	if(req->actual % dev->ep_out->maxpacket == 0 && xfer != count) {
		goto requeue_req;
	} else {
		r = xfer;
	}
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
	if(r > 0){
		switch (dev->cdev->gadget->speed) {
			case USB_SPEED_LOW:
			case USB_SPEED_FULL:
				max_packet_size = acc_fullspeed_in_desc.wMaxPacketSize;
				break;
			case USB_SPEED_HIGH:
				max_packet_size = acc_highspeed_in_desc.wMaxPacketSize;
				break;
			default:
				max_packet_size = acc_highspeed_in_desc.wMaxPacketSize;
				break;
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
	if(_acc_dev->queue_flg)
		usb_ep_dequeue(_acc_dev->ep_out, _acc_dev->rx_req[0]);
	
	_acc_dev->queue_flg = 0;
#endif /* CONFIG_USB_ANDROID_SH_CUST */
	_acc_dev->disconnected = 0;
	return 0;
}
