...
#define EPT_PRIME_CHECK_DELAY	(jiffies + msecs_to_jiffies(3))
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
	if (atomic_read(&ui->self_powered))
		return 0;
#endif /* CONFIG_USB_ANDROID_SH_CUST */
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
				spin_lock_irqsave(&ui->lock, iflags);
				if (ui->flags & USB_FLAG_RESET) {
					dev_dbg(&ui->pdev->dev,
						"msm72k_udc: maybe disconnected,so cancel reset\n");
...
static int msm72k_pullup_internal(struct usb_gadget *_gadget, int is_active)
{
	...
	if (is_active) {
		...
		if ((ui->state == USB_STATE_ONLINE) &&
			((otg_io_read(ui->xceiv, 0x04) & 0x18) != 0x00)) {
			ui->flags |= USB_FLAG_RESET;
			schedule_work(&ui->work);
		}
#endif /* CONFIG_USB_ANDROID_SH_CUST */
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
int msm72k_set_fullspeed_connect( struct usb_gadget *_gadget, int enable )
{
	struct usb_info *ui = container_of(_gadget, struct usb_info, gadget);
