...
#endif /* CONFIG_USB_MSM_OTG_72K_SH_CUST */

#ifdef CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC
    #include <sharp/shswic_kerl.h>
    #if !defined(CONFIG_USB_EHCI_HCD)
        #define D_USB_HOST_CABLE_NOT_SUPPORTED
    #endif /* !defined(CONFIG_USB_EHCI_HCD) */
#endif /* CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC */
...
#ifdef CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION
    #define D_CHECK_LEGACY_DETECTION_WAIT_MS    (1000)
#endif /* CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION */
#ifdef CONFIG_USB_MSM_OTG_72K_SH_CUST
    #define D_CHECK_CABLE_FAIL_SAFE_WAIT_MS     (5000)
#endif /* CONFIG_USB_MSM_OTG_72K_SH_CUST */
...
#ifdef CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC
static atomic_t enable_shswic_notif = ATOMIC_INIT(0);

static void msm_otg_shswic_notify(u8 device,struct msm_otg *dev)
{
    int work = 0;
    unsigned long flags;
    int enable_notif = 0;
    ...
    enable_notif = atomic_read(&enable_shswic_notif);
    switch(device) {
    case SHSWIC_ID_USB_CABLE:
        pr_debug("%s: USB Cable Connect set\n", __func__);
        dev->chg_det_type = CHG_DET_SDP;
        if (enable_notif) {
    ...
    case SHSWIC_ID_NONE:
        ...
        dev->chg_det_type = CHG_DET_INVALID;
        clear_bit(B_SESS_VLD, &dev->inputs);
        set_bit(ID, &dev->inputs);
        work = 1;
        break;
#ifndef D_USB_HOST_CABLE_NOT_SUPPORTED
    case SHSWIC_ID_USB_HOST_CABLE:
        pr_debug("%s: Host Cable Connect set\n", __func__);
        dev->chg_det_type = CHG_DET_INVALID;
        if (enable_notif) {
            clear_bit(B_SESS_VLD, &dev->inputs);
        ...
        break;
#endif /* D_USB_HOST_CABLE_NOT_SUPPORTED */
    default:
...
static void msm_otg_shswic_irq(u8 device, void *data)
{
    struct msm_otg *dev;
    ...
    msm_otg_shswic_notify(device,dev);

    return;
}

static void ext_notif_stop(void)
{
...
static int ext_notif_start(struct msm_otg *dev)
{
    shswic_result_t swic_ret;

    swic_ret = shswic_detect_cb_regist(SHSWIC_VBUS_DEVICE,
#ifndef D_USB_HOST_CABLE_NOT_SUPPORTED
            (SHSWIC_ID_USB_CABLE | SHSWIC_ID_AC_ADAPTER | SHSWIC_ID_USB_HOST_CABLE),
#else /* D_USB_HOST_CABLE_NOT_SUPPORTED */
            (SHSWIC_ID_USB_CABLE | SHSWIC_ID_AC_ADAPTER),
#endif /* D_USB_HOST_CABLE_NOT_SUPPORTED */
            (void*)msm_otg_shswic_irq, dev);
    if (swic_ret != SHSWIC_SUCCESS) {
        pr_err("%s: shswic_detect_cb_regist err\n", __func__);
        return -1;
    }
...
{
#ifdef CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC
    atomic_set(&enable_shswic_notif,1);
#endif /* CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC */
    /* Do nothing if instead of ID pin, USER controls mode switch */
    if (dev->pdata->otg_mode == OTG_USER_CONTROL)
        return;
...
{
#ifdef CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC
    atomic_set(&enable_shswic_notif,0);
#endif /* CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC */
    /* Do nothing if instead of ID pin, USER controls mode switch */
    if (dev->pdata->otg_mode == OTG_USER_CONTROL)
        return;
...
#ifndef CONFIG_USB_MSM_OTG_72K_SH_CUST
...
#else /* CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC */
    if (readl_relaxed(USB_OTGSC) & OTGSC_BSVIS) {
#endif /* CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC */
        set_bit(B_SESS_VLD, &dev->inputs);
        queue_work(dev->wq, &dev->sm_work);
        ...
        queue_delayed_work(dev->wq, &dev->fsafe_work,
                         msecs_to_jiffies(D_CHECK_CABLE_FAIL_SAFE_WAIT_MS));
#endif /* CONFIG_USB_MSM_OTG_72K_SH_CUST */
...
#ifdef CONFIG_USB_MSM_OTG_72K_SH_CUST
static void msm_otg_fail_safe_work(struct work_struct *w)
{
    struct msm_otg *dev = container_of(w, struct msm_otg, fsafe_work.work);

    pr_info("%s: retry a cable check\n", __func__);
    queue_work(dev->wq, &dev->sm_work);
}
#endif /* CONFIG_USB_MSM_OTG_72K_SH_CUST */
...
#define D_CHECK_DCD_COUNT_MAX           (3)
#define D_CHECK_DCD_FIRST_CHECK_WAIT_MS     (10)
#define D_CHECK_DCD_RECHECK_WAIT_MS     (500)

static int dcd_check_count = 0;
static unsigned int chg_det_fctl_val = 0;
static int non_driving_mode = 0;

inline static void start_non_driving_mode_if_need(struct msm_otg *dev)
{
    if (non_driving_mode)
        return;

    if (chg_det_fctl_val != 0)
        printk("%s:chg_det_fctl_val is not reset (0x%x)\n",__func__, chg_det_fctl_val);

    chg_det_fctl_val = ulpi_read(dev, ULPI_FUNCTION_CONTROL_REG);

    if( (chg_det_fctl_val & 0x18) != 0x08 ) {
        /* if not non-driving mode,then change non-driving mode */
        ulpi_write(dev, (chg_det_fctl_val & ~0x18 ) | 0x08 , ULPI_FUNCTION_CONTROL_REG);
    }

    non_driving_mode = 1;
}

inline static void stop_non_driving_mode_if_need(struct msm_otg *dev)
{
    if( (chg_det_fctl_val & 0x18) != 0x08 ) {
        ulpi_write(dev, chg_det_fctl_val , ULPI_FUNCTION_CONTROL_REG);
    }
    chg_det_fctl_val = 0;
    non_driving_mode = 0;
}

inline static void reset_dcd_check_count(void)
{
    dcd_check_count = 0;
}

#ifndef CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION
inline static int is_over_after_countup_dcd_check_count(void)
{
    if (dcd_check_count++ > D_CHECK_DCD_COUNT_MAX)
        return 1;
    else
        return 0;
}
...
static int check_device_mode_detect_vout_high(struct msm_otg *dev)
{
    unsigned int chg_det;
    int vout;

    ulpi_write(dev, 0x0F , ULPI_CHG_DETECT_REG);
    udelay(50); /* wait */

    ulpi_write(dev, 0x07 , ULPI_CHG_DETECT_REG);

    ulpi_write(dev, 0x05 , ULPI_CHG_DETECT_REG);
    udelay(50); /* wait */

    ulpi_write(dev, 0x04 , ULPI_CHG_DETECT_REG);
    udelay(50); /* wait */
...
#ifndef CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION
static int check_host_mode_detect_vout_high(struct msm_otg *dev)
{
    unsigned int chg_det;
    int vout;
    ...
    ulpi_write(dev, 0x03 , ULPI_CHG_DETECT_REG);

    ulpi_write(dev, 0x01 , ULPI_CHG_DETECT_REG);
    udelay(50); /* wait */

    ulpi_write(dev, 0x00 , ULPI_CHG_DETECT_REG);
    udelay(50); /* wait */
...
/* function:enable_dcd */
/* inputs  :struct msm_otg *dev         */
/* output  :result                      */
static void enable_dcd(struct msm_otg *dev)
{
    /* enable dcd */
    ulpi_write(dev, 0x0F , ULPI_CHG_DETECT_REG);
    udelay(50); /* wait */
    ...
    ulpi_write(dev, 0x25 , ULPI_CHG_DETECT_REG);
}

static enum usb_sh_chg_det_state check_dcd(struct msm_otg *dev)
{
    unsigned int line_state;
    enum usb_sh_chg_det_state ret;
    int vout;
    ...
    switch (line_state) {
    case 0:                 /* SE0 */
        ret = CHG_DET_DCD_DETECT;
        break;
    case (3 << 10):         /* SE1 */
        /* check host-charger */
        vout = check_host_mode_detect_vout_high(dev);
        if (vout)
            ret = CHG_DET_DCP;
        else
            ret = CHG_DET_DCD_WAIT_FOR_DP_LOW;
        break;
    case (1 << 10):
    default:
        ret = CHG_DET_DCD_WAIT_FOR_DP_LOW;
...
static void disable_dcd(struct msm_otg *dev)
{
    ulpi_write(dev, 0x0F , ULPI_CHG_DETECT_REG);
    udelay(50); /* wait */
}

static enum usb_sh_chg_det_state check_charger_type(struct msm_otg *dev ,enum usb_sh_chg_det_state state)
{
    enum usb_sh_chg_det_state ret;
    int vout;

    vout = check_device_mode_detect_vout_high(dev);
...
static void msm_chg_sh_detect_work(struct work_struct *w)
{
    unsigned long flags;
    enum usb_sh_chg_det_state notify_state = CHG_DET_NOSET;
    struct msm_otg *dev = container_of(w, struct msm_otg, chg_work.work);
#ifndef CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION
    enum usb_sh_chg_det_state ret_dcd;
#endif /* CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION */

#ifdef CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC
    if (dev->pdata->otg_mode == OTG_USER_CONTROL) {
        shswic_result_t ret_swic;
        u8 device;

        ret_swic = shswic_get_usb_port_status(&device);
        if (ret_swic != SHSWIC_SUCCESS)
            device = SHSWIC_ID_NONE;
        dev->chg_stage = CHG_DET_STAGE_COMPLETE;
        msm_otg_shswic_notify(device,dev);
        return;
    }
#endif /* CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC */
    ...
    switch (dev->chg_stage) {
    case CHG_DET_STAGE_NOT_START:
#ifdef CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION
        start_non_driving_mode_if_need(dev);
        if (check_device_mode_detect_vout_high(dev))
            notify_state = CHG_DET_DCP;
        else
            notify_state = CHG_DET_SDP;
        stop_non_driving_mode_if_need(dev);
        dev->chg_stage = CHG_DET_STAGE_COMPLETE;
        break;
#else /* CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION */
        start_non_driving_mode_if_need(dev);
        enable_dcd(dev);
        dev->chg_stage = CHG_DET_STAGE_WAIT_FOR_DCD;
        queue_delayed_work(dev->wq, &dev->chg_work,
                         msecs_to_jiffies(D_CHECK_DCD_FIRST_CHECK_WAIT_MS));
        break;
    case CHG_DET_STAGE_WAIT_FOR_DCD:
        ret_dcd = check_dcd(dev);
        switch (ret_dcd) {
        case CHG_DET_DCD_WAIT_FOR_DP_LOW:
            if (is_over_after_countup_dcd_check_count() == 0) {
                queue_delayed_work(dev->wq, &dev->chg_work,
                         msecs_to_jiffies(D_CHECK_DCD_RECHECK_WAIT_MS));
            }
            else {
                disable_dcd(dev);
                notify_state = check_charger_type(dev,CHG_DET_DCD_NOT_DETECT);
                stop_non_driving_mode_if_need(dev);
                dev->chg_stage = CHG_DET_STAGE_COMPLETE;
            }
            break;
        case CHG_DET_DCD_DETECT:
            disable_dcd(dev);
            notify_state = check_charger_type(dev,CHG_DET_DCD_DETECT);
            stop_non_driving_mode_if_need(dev);
            dev->chg_stage = CHG_DET_STAGE_COMPLETE;
            break;
        case CHG_DET_DCP:
            disable_dcd(dev);
            notify_state = ret_dcd;
            stop_non_driving_mode_if_need(dev);
            dev->chg_stage = CHG_DET_STAGE_COMPLETE;
            break;
        default:
        ...
        break;
#endif /* CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION */
    case CHG_DET_STAGE_COMPLETE:
    default:
        break;
    }

    if (notify_state != CHG_DET_NOSET) {
        dev->chg_det_type = notify_state;
        queue_work(dev->wq, &dev->sm_work);
    }
...
static void msm_otg_change_cable_notify(void)
{
    ...
    kobject_uevent_env(&dev->otg.dev->kobj,KOBJ_CHANGE, uevent_envp);
}
#endif /* CONFIG_USB_MSM_OTG_72K_SH_CUST */

#ifdef CONFIG_USB_MSM_OTG_72K_SH_CUST
static void msm_chg_reset(struct msm_otg *dev)
{
...
#endif /* CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION */
    default:
        pr_err("%s:invalid stage(0x%x)\n",__func__, dev->chg_stage);
        break;
    }
    reset_dcd_check_count();

#ifdef CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC
    if (dev->pdata->otg_mode != OTG_USER_CONTROL) {
        dev->chg_stage = CHG_DET_STAGE_NOT_START;
        dev->chg_det_type = CHG_DET_NOSET;
    }
#else /* CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC */
    dev->chg_stage = CHG_DET_STAGE_NOT_START;
    dev->chg_det_type = CHG_DET_NOSET;
...
static void msm_otg_sm_work(struct work_struct *w)
{
...
#ifdef CONFIG_USB_MSM_OTG_72K_SH_CUST
    cancel_delayed_work_sync(&dev->fsafe_work);
#endif /* CONFIG_USB_MSM_OTG_72K_SH_CUST */
    ...
    case OTG_STATE_UNDEFINED:
        ...
        if (dev->pdata->otg_mode == OTG_USER_CONTROL) {
            ...
            clear_bit(B_SESS_VLD, &dev->inputs);
            set_bit(ID, &dev->inputs);
            queue_delayed_work(dev->wq, &dev->chg_work, 0);
#endif /* CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC */
        } else {
    ...
    case OTG_STATE_B_IDLE:
        ...
        if (!test_bit(ID, &dev->inputs) ||
...
#ifdef CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION
                queue_delayed_work(dev->wq, &dev->chg_work,
                         msecs_to_jiffies(D_CHECK_LEGACY_DETECTION_WAIT_MS));
#else /* CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION */
                queue_delayed_work(dev->wq, &dev->chg_work,
                         msecs_to_jiffies(D_CHECK_DCD_FIRST_CHECK_WAIT_MS));
#endif /* CONFIG_USB_MSM_OTG_72K_USE_LEGACY_DETECTION */
            ...
            case CHG_DET_STAGE_COMPLETE:
                ...
                switch (dev->chg_det_type) {
                case CHG_DET_DCP:
                    atomic_set(&dev->chg_type, USB_CHG_TYPE__WALLCHARGER);
                    /* value of current is copied from
                       USB_WALLCHARGER_CHG_CURRENT at msm72k_udc.c */
                    msm_otg_set_power(&dev->otg, 1800);
                    break;
                case CHG_DET_SDP:
                case CHG_DET_CPD:
                ...
                break;
            default:
                printk("%s:invalid stage(0x%x)\n",__func__, dev->chg_stage);
                break;
            }
#else /* CONFIG_USB_MSM_OTG_72K_SH_CUST */
        } else if (test_bit(B_SESS_VLD, &dev->inputs) &&
                !test_bit(ID_B, &dev->inputs)) {
            pr_debug("b_sess_vld\n");
            spin_lock_irqsave(&dev->lock, flags);
...
#ifdef CONFIG_USB_MSM_OTG_72K_SH_CUST
            if (readl(USB_OTGSC) & OTGSC_BSV) {
                /* an unsupported cable is connected,so stay in A_IDLE */
                pr_err("%s: an unsupported cable is connected\n", __func__);
                break;
            }
#endif /* CONFIG_USB_MSM_OTG_72K_SH_CUST */
    ...
    if (!work_pending(&dev->sm_work) && !hrtimer_active(&dev->timer) &&
            ...
            !work_pending(&dev->chg_work.work))
#else /* CONFIG_USB_MSM_OTG_72K_SH_CUST */
    if (!work_pending(&dev->sm_work) && !hrtimer_active(&dev->timer) &&
            !work_pending(&dev->otg_resume_work))
#endif /* CONFIG_USB_MSM_OTG_72K_SH_CUST */
            ...
            const char *buf, size_t size)
{
    ...
    case '0':
        if (shvbus_usb_force_disconnect == 1) {
            dev->pmic_vbus_notif_supp = 0;
#ifdef CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC
            ret = ext_notif_start(dev);
            if (ret) {
                pr_err("%s: external notification cant start\n", __func__);
                pr_err("%s: restart detection by msm\n", __func__);
                dev->pdata->otg_mode = OTG_ID;
            }
            else {
                dev->pdata->pmic_vbus_notif_init = NULL;
                dev->pdata->pmic_id_notif_init = NULL;
            ...
            spin_lock_irqsave(&dev->lock, flags);
#ifndef CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC
            dev->pdata->otg_mode = OTG_ID;
#endif /* CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC */
            dev->otg.state = OTG_STATE_UNDEFINED;
            spin_unlock_irqrestore(&dev->lock, flags);
...
static ssize_t
show_usb_cable_type(struct device *sdev, struct device_attribute *attr,
            char *buf)
{
    size_t val;
    struct msm_otg *dev = the_msm_otg;
    ...
    val = snprintf(buf, PAGE_SIZE, "%s", cable_str[cable_type]);
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_USB_MSM_OTG_72K_SH_CUST
    INIT_DELAYED_WORK(&dev->chg_work, msm_chg_sh_detect_work);
    INIT_DELAYED_WORK(&dev->fsafe_work, msm_otg_fail_safe_work);
#endif /* CONFIG_USB_MSM_OTG_72K_SH_CUST */
...
#ifdef CONFIG_USB_MSM_OTG_72K_USE_USB_SWIC
    /* use shswic to detect VBUS session change */
    dev->pdata->otg_mode = OTG_USER_CONTROL;
    ...
    if (ret < 0) {
        pr_err("%s: Failed to create the file entry(usb_force_disconnect)\n", __func__);
        goto remove_group;
    }
