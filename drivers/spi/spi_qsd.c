...
#define	SUSPEND_RETRY	5

#define SH_SPI_ACTIVE_REQ_NONE		0
#define SH_SPI_ACTIVE_REQ_SPI_CORE	(1 << 0)
#define SH_SPI_ACTIVE_REQ_TPS		(1 << 1)
#define SH_SPI_ACTIVE_REQ_SUBLCD	(1 << 2)
#define SH_SPI_ACTIVE_REQ_ALL		0xFFFFFFFF
#endif	/* CONFIG_SPI_CUST_SH */
...
#if defined( CONFIG_SPI_CUST_SH )
static void sh_spi_clk_enable(struct msm_spi *dd)
{
	if(dd->clk_state == 0){
		clk_enable(dd->clk);
		if (dd->pclk){
...
static void sh_spi_clk_disable(struct msm_spi *dd)
{
	if(dd->clk_state != 0){
		clk_disable(dd->clk);
		if (dd->pclk){
...
static void sh_spi_active_func(struct msm_spi *dd, uint32_t active)
{
	mutex_lock(&dd->mlock);

	if(dd->spi_active == SH_SPI_ACTIVE_REQ_NONE){
		sh_spi_clk_enable(dd);
		dd->pdata->gpio_config();
	}

	dd->spi_active |= active;

	mutex_unlock(&dd->mlock);
}

static void sh_spi_standby_func(struct msm_spi *dd, uint32_t active)
{
	mutex_lock(&dd->mlock);
...
void sh_spi_tps_active(struct spi_device *spi)
{
	struct msm_spi *dd = spi_master_get_devdata(spi->master);

	if(dd != NULL){
		sh_spi_active_func(dd, SH_SPI_ACTIVE_REQ_TPS);
	}
}
EXPORT_SYMBOL(sh_spi_tps_active);

void sh_spi_tps_standby(struct spi_device *spi)
{
	struct msm_spi *dd = spi_master_get_devdata(spi->master);

	if(dd != NULL){
		sh_spi_standby_func(dd, SH_SPI_ACTIVE_REQ_TPS);
	}
}
EXPORT_SYMBOL(sh_spi_tps_standby);

void sh_spi_sublcd_active(struct spi_device *spi)
{
	struct msm_spi *dd = spi_master_get_devdata(spi->master);

	if(dd != NULL){
		sh_spi_active_func(dd, SH_SPI_ACTIVE_REQ_SUBLCD);
	}
}
EXPORT_SYMBOL(sh_spi_sublcd_active);

void sh_spi_sublcd_standby(struct spi_device *spi)
{
	struct msm_spi *dd = spi_master_get_devdata(spi->master);

	if(dd != NULL){
		sh_spi_standby_func(dd, SH_SPI_ACTIVE_REQ_SUBLCD);
	}
}
EXPORT_SYMBOL(sh_spi_sublcd_standby);
#endif	/* CONFIG_SPI_CUST_SH */
...
#if defined( CONFIG_SPI_CUST_SH )
	if (dd->cur_msg->spi->bits_per_word > 32) {
		dev_err(&dd->cur_msg->spi->dev, "%s: invalid bits_per_word %d\n",
			__func__, dd->cur_msg->spi->bits_per_word);
		dd->cur_msg->status = -EINVAL;
		return;
	}

	if (dd->cur_msg->spi->chip_select > SPI_NUM_CHIPSELECTS-1) {
		dev_err(&dd->cur_msg->spi->dev, "%s, chip select %d exceeds max value %d\n",
			__func__, dd->cur_msg->spi->chip_select, SPI_NUM_CHIPSELECTS - 1);
		dd->cur_msg->status = -EINVAL;
		return;
	}
#endif	/* CONFIG_SPI_CUST_SH */
...
#if defined( CONFIG_SPI_CUST_SH )
	mask = SPI_IO_C_CS_N_POLARITY_0 << dd->cur_msg->spi->chip_select;
...
#if defined( CONFIG_SPI_CUST_SH )
	writel_relaxed((dd->cur_transfer->delay_usecs & 0x3f), dd->base + SPI_DEASSERT_WAIT);
#endif	/* CONFIG_SPI_CUST_SH */
...
#if defined( CONFIG_SPI_CUST_SH )
	sh_spi_active_func(dd, SH_SPI_ACTIVE_REQ_SPI_CORE);
#else
	clk_enable(dd->clk);
	clk_enable(dd->pclk);
#endif	/* CONFIG_SPI_CUST_SH */
...
#if defined( CONFIG_SPI_CUST_SH )
	sh_spi_standby_func(dd, SH_SPI_ACTIVE_REQ_SPI_CORE);
#else
	clk_disable(dd->clk);
	clk_disable(dd->pclk);
#endif	/* CONFIG_SPI_CUST_SH */
...
#if defined( CONFIG_SPI_CUST_SH )
	{
		for (suspend_cnt=0; suspend_cnt<=SUSPEND_RETRY; suspend_cnt++) {
			msleep(20);
			if(dd->suspended==0)
				break;
		}
		if(suspend_cnt==SUSPEND_RETRY)
		{
				return -EBUSY;
		}
	}
...
skip_dma_resources:
	...
	mutex_init(&dd->mlock);
	dd->clk_state = 0;
	dd->spi_active = SH_SPI_ACTIVE_REQ_NONE;
#endif	/* CONFIG_SPI_CUST_SH */
...
static int msm_spi_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	mutex_lock(&dd->mlock);
	if(dd->spi_active != SH_SPI_ACTIVE_REQ_NONE){
		printk(KERN_INFO "[MSM-SPI] suspend with active on (0x%08X)\n", dd->spi_active);
	}
	mutex_unlock(&dd->mlock);

	sh_spi_standby_func(dd, SH_SPI_ACTIVE_REQ_ALL);
#endif	/* CONFIG_SPI_CUST_SH */
	msm_spi_free_gpios(dd);
suspend_exit:
	return 0;
