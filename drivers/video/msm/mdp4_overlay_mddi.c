				...
				struct mdp4_overlay_pipe *pipe)
{
	...
	/* wait until DMA finishes the current job */
  	if (!wait_for_completion_timeout(&pipe->comp, msecs_to_jiffies(100))) {
		extern int mdp4_overlay_running_perf_level;
		printk(KERN_ERR "%s: completion_timeout\n", __func__);
		pr_err("[++++]%s:**err**:src_format=%d, src_width=%d, src_height=%d, src=[%d,%d,%d,%d], dst=[%d,%d,%d,%d], running_perf_level=%d\n", __func__, pipe->src_format, pipe->src_width, pipe->src_height, pipe->src_x, pipe->src_y, pipe->src_w, pipe->src_h, pipe->dst_x, pipe->dst_y, pipe->dst_w, pipe->dst_h, mdp4_overlay_running_perf_level);
		shlcdc_errlog_rec(0x001F0000, mdp4_overlay_running_perf_level);
