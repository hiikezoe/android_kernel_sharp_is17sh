...
static int mddi_sharp_klog=0;
static int mddi_sharp_lcd_on_off=0;
static int mddi_sharp_lcd_powmonitor=0;
#ifdef CONFIG_ANDROID_ENGINEERING
module_param(mddi_sharp_klog, int, 0600);
module_param(mddi_sharp_lcd_on_off, int, 0400);
module_param(mddi_sharp_lcd_powmonitor, int, 0400);
#endif
#define pr_info_lv(fmt, arg...)	\
	if(mddi_sharp_klog>0) pr_info(fmt, ## arg);

#ifndef SH_BUILD_ID
#define	SH_BUILD_ID	"S0000"
#endif

enum enum_mddi_sharp_event {
	EEVENT_MDDI_SHARP_LCD_POWERDOWN,
	EEVENT_MDDI_SHARP_LCD_SET_BACKLIGHT,
	EEVENT_MDDI_SHARP_PRIM_LCD_INIT,
	EEVENT_MDDI_SHARP_LCD_ON,
	EEVENT_MDDI_SHARP_LCD_OFF,
};

#define	EVENTLOG_MDDI_SHARP	0x300
static void mddi_sharp_eventlog_rec(unsigned long event, unsigned long param)
{
	shlcdc_eventlog_rec(SHLCDC_EVENTLOG_FB + EVENTLOG_MDDI_SHARP + event, param);
}

static void mddi_sharp_kmsg_dumper(struct kmsg_dumper *dumper,
		enum kmsg_dump_reason reason, const char *s1, unsigned long l1,
		const char *s2, unsigned long l2)
{
	if(reason == KMSG_DUMP_PANIC) return;
	pr_info("[****]%s:reason=%d, mddi_sharp_lcd_on_off=%d, mddi_sharp_lcd_powmonitor=%d\n", __func__, reason, mddi_sharp_lcd_on_off, mddi_sharp_lcd_powmonitor);
}
static struct kmsg_dumper kmsg_dumper_mddi_sharp = {
	.dump = mddi_sharp_kmsg_dumper,
};

static int mddi_sharp_notifier_call(struct notifier_block *this,
				  unsigned long code,
				  void *_cmd)
{
	pr_info("[****]%s:code=%lu, mddi_sharp_lcd_on_off=%d, mddi_sharp_lcd_powmonitor=%d\n", __func__, code, mddi_sharp_lcd_on_off, mddi_sharp_lcd_powmonitor);
	return NOTIFY_DONE;
}

static struct notifier_block nb_mddi_sharp = {
	.notifier_call = mddi_sharp_notifier_call,
};
...
{
#ifdef CONFIG_SHLCDC_BOARD
	pr_info_lv("[****]%s", __func__);
	mddi_sharp_lcd_powmonitor=1;
	mddi_sharp_eventlog_rec(EEVENT_MDDI_SHARP_LCD_POWERDOWN, 0);
...
#ifdef CONFIG_SHLCDC_BOARD
  pr_info_lv("[****]%s", __func__);
  mddi_sharp_eventlog_rec(EEVENT_MDDI_SHARP_LCD_SET_BACKLIGHT, mfd->bl_level);
  if (mfd->bl_level == 0) {
...
{
#ifdef CONFIG_SHLCDC_BOARD
	mddi_sharp_lcd_powmonitor=0;
	mddi_sharp_eventlog_rec(EEVENT_MDDI_SHARP_PRIM_LCD_INIT, 0);
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_lv("[****]%s", __func__);
	mddi_sharp_lcd_on_off=1;
	mddi_sharp_eventlog_rec(EEVENT_MDDI_SHARP_LCD_ON, 0);
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_lv("[****]%s", __func__);
	mddi_sharp_lcd_on_off=0;
	mddi_sharp_eventlog_rec(EEVENT_MDDI_SHARP_LCD_OFF, 0);
#endif	/* CONFIG_SHLCDC_BOARD */
...
static int __init mddi_sharp_init(void)
{
	...
	if(strncmp(SH_BUILD_ID, "S", 1) != 0){
		kmsg_dump_register(&kmsg_dumper_mddi_sharp);
		modem_register_notifier(&nb_mddi_sharp);
	}
#endif	/* CONFIG_SHLCDC_BOARD */
