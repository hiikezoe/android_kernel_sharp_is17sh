...
module_param(mdp_suspended, bool, 0400);
#endif
...
static int mdp_klog=0;
static int mdp_klog_irq=0;
static int mdp_early_suspend_resume=0;
static int mdp_on_off=0;
#ifdef CONFIG_ANDROID_ENGINEERING
module_param(mdp_klog, int, 0600);
module_param(mdp_klog_irq, int, 0600);
module_param(mdp_early_suspend_resume, int, 0400);
module_param(mdp_on_off, int, 0400);
#endif
#define pr_info_klog(fmt, arg...)	\
	if(mdp_klog>0) pr_info(fmt, ## arg);
#define pr_info_klog_irq(fmt, arg...)	\
	if(mdp_klog_irq>0) pr_info(fmt, ## arg);
...
enum enum_mdp_event{
	EEVENT_MDP_OFF,
	EEVENT_MDP_ON,
	EEVENT_MDP_SUSPEND_SHDISP,
	EEVENT_MDP_SUSPEND,
	EEVENT_MDP_EARLY_SUSPEND,
	EEVENT_MDP_EARLY_RESUME,
};

#define	EVENTLOG_MDP	0x200
static void mdp_eventlog_rec(unsigned long event, unsigned long param)
{
	shlcdc_eventlog_rec(SHLCDC_EVENTLOG_FB + EVENTLOG_MDP + event, param);
}

static void mdp_kmsg_dumper(struct kmsg_dumper *dumper,
		enum kmsg_dump_reason reason, const char *s1, unsigned long l1,
		const char *s2, unsigned long l2)
{
	if(reason == KMSG_DUMP_PANIC) return;
	pr_info("[----]%s:reason=%d, mdp_early_suspend_resume=%d, mdp_on_off=%d, mdp_suspended=%d\n", __func__, reason, mdp_early_suspend_resume, mdp_on_off, mdp_suspended);
}
static struct kmsg_dumper kmsg_dumper_mdp = {
	.dump = mdp_kmsg_dumper,
};

static int mdp_notifier_call(struct notifier_block *this,
				  unsigned long code,
				  void *_cmd)
{
	pr_info("[----]%s:code=%lu, mdp_early_suspend_resume=%d, mdp_on_off=%d, mdp_suspended=%d\n", __func__, code, mdp_early_suspend_resume, mdp_on_off, mdp_suspended);
	return NOTIFY_DONE;
}

static struct notifier_block nb_mdp = {
	.notifier_call = mdp_notifier_call,
};
...
int _mdp_histogram_ctrl(boolean en, struct mdp_hist_mgmt *mgmt)
{
	...
	hist_start = mgmt->mdp_is_hist_start;
...
static int mdp_off(struct platform_device *pdev)
{
	...
	pr_info_klog("[----]%s\n", __func__);
	mdp_on_off=0;
	mdp_eventlog_rec(EEVENT_MDP_OFF, 0);
#endif	/* CONFIG_SHLCDC_BOARD */
...
static int mdp_on(struct platform_device *pdev)
{
	...
	pr_info_klog("[----]%s\n", __func__);
	mdp_on_off=1;
	mdp_eventlog_rec(EEVENT_MDP_ON, 0);
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[----]%s:sw=%d\n", __func__, sw);
    mdp_eventlog_rec(EEVENT_MDP_SUSPEND_SHDISP, sw);
#endif	/* CONFIG_SHLCDC_BOARD */
    if (sw == TRUE)
        mdp_suspend_sub();
    return;
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[----]%s\n", __func__);
	mdp_eventlog_rec(EEVENT_MDP_SUSPEND, 0);
#endif	/* CONFIG_SHLCDC_BOARD */
	if (pdev->id == 0) {
		mdp_suspend_sub();
		if (mdp_current_clk_on) {
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[----]%s\n", __func__);
	mdp_early_suspend_resume=1;
	mdp_eventlog_rec(EEVENT_MDP_EARLY_SUSPEND, 0);
#endif	/* CONFIG_SHLCDC_BOARD */
	mdp_suspend_sub();
#ifdef CONFIG_FB_MSM_DTV
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[----]%s\n", __func__);
	mdp_early_suspend_resume=0;
	mdp_eventlog_rec(EEVENT_MDP_EARLY_RESUME, 0);
#endif	/* CONFIG_SHLCDC_BOARD */
...
static int __init mdp_driver_init(void)
{
	...
	if(strncmp(SH_BUILD_ID, "S", 1) != 0){
		kmsg_dump_register(&kmsg_dumper_mdp);
		modem_register_notifier(&nb_mdp);
	}
#endif	/* CONFIG_SHLCDC_BOARD */
