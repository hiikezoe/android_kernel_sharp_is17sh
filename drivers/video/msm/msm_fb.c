...
static int msm_fb_klog=0;
static int msm_fb_klog_overlay=0;
static int msm_fb_verbose=0;
static int msm_fb_pandisp=0;
static int msm_fb_suspend_resume=0;
static int msm_fb_early_suspend_resume=0;
static unsigned long msm_fb_last_ioctl_cmd=0;
#ifdef CONFIG_ANDROID_ENGINEERING
module_param(msm_fb_klog, int, 0600);
module_param(msm_fb_klog_overlay, int, 0600);
module_param(msm_fb_verbose, int, 0600);
module_param(msm_fb_pandisp, int, 0600);
module_param(msm_fb_suspend_resume, int, 0400);
module_param(msm_fb_early_suspend_resume, int, 0400);
module_param(msm_fb_last_ioctl_cmd, ulong, 0400);
#endif
#define pr_info_klog(fmt, arg...)	\
	if(msm_fb_klog>0) pr_info(fmt, ## arg);
#define pr_info_klog_overlay(fmt, arg...)	\
	if(msm_fb_klog_overlay>0) pr_info(fmt, ## arg);
#define pr_info_verbose(fmt, arg...)	\
	if(msm_fb_verbose>0) pr_info(fmt, ## arg);
#define pr_info_pandisp(fmt, arg...)	\
	if(msm_fb_pandisp>0) pr_info(fmt, ## arg);
...
enum enum_msm_fb {
	EEVENT_MSM_FB_IOCTL,
	EEVENT_MSM_FB_GET_WRITEBACK_FB,
	EEVENT_MSM_FB_WRITEBACK_START,
	EEVENT_MSM_FB_WRITEBACK_QUEUE_BUFFER,
	EEVENT_MSM_FB_WRITEBACK_DEQUEUE_BUFFER,
	EEVENT_MSM_FB_WRITEBACK_STOP,
	EEVENT_MSM_FB_WRITEBACK_INIT,
	EEVENT_MSM_FB_WRITEBACK_TERMINATE,
	EEVENT_MSM_FB_SET_BRIGHTNESS,
	EEVENT_MSM_FB_SUSPEND,
	EEVENT_MSM_FB_RESUME,
	EEVENT_MSM_FB_EARLY_SUSPEND,
	EEVENT_MSM_FB_EARLY_RESUME,
	EEVENT_MSM_FB_SET_BACKLIGHT,
	EEVENT_MSM_FB_BLANK_SUB,
	EEVENT_MSM_FB_PAN_DISPLAY,
};

#define	EVENTLOG_MSM_FB	0x00
static void msm_fb_eventlog_rec(unsigned long event, unsigned long param)
{
	shlcdc_eventlog_rec(SHLCDC_EVENTLOG_FB + EVENTLOG_MSM_FB + event, param);
}

static void msm_fb_busylog_rec(unsigned long event, unsigned long param)
{
	shlcdc_busylog_rec(SHLCDC_EVENTLOG_FB + EVENTLOG_MSM_FB + event, param);
}

static void msm_fb_kmsg_dumper(struct kmsg_dumper *dumper,
		enum kmsg_dump_reason reason, const char *s1, unsigned long l1,
		const char *s2, unsigned long l2)
{
	if(reason == KMSG_DUMP_PANIC) return;
	pr_info("[====]%s:reason=%d, msm_fb_early_suspend_resume=%d, msm_fb_suspend_resume=%d, msm_fb_last_ioctl_cmd=0x%lxn", __func__, reason, msm_fb_early_suspend_resume, msm_fb_suspend_resume, msm_fb_last_ioctl_cmd);
}
static struct kmsg_dumper kmsg_dumper_msm_fb = {
	.dump = msm_fb_kmsg_dumper,
};

static int msm_fb_notifier_call(struct notifier_block *this,
				  unsigned long code,
				  void *_cmd)
{
	pr_info("[====]%s:msm_fb_early_suspend_resume=%d, msm_fb_suspend_resume=%d, msm_fb_last_ioctl_cmd=0x%lx\n", __func__, msm_fb_early_suspend_resume, msm_fb_suspend_resume, msm_fb_last_ioctl_cmd);
	return NOTIFY_DONE;
}

static struct notifier_block nb_msm_fb = {
	.notifier_call = msm_fb_notifier_call,
};
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_ANDROID_ENGINEERING
module_param(msm_fb_suspend_flag_shdisp, int, 0400);
#endif
...
#ifdef	CONFIG_SHLCDC_BOARD
	msm_fb_eventlog_rec(EEVENT_MSM_FB_SET_BRIGHTNESS, value);
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[====]%s:\n", __func__);
	msm_fb_suspend_resume=1;
	msm_fb_eventlog_rec(EEVENT_MSM_FB_SUSPEND, state);
#endif	/* CONFIG_SHLCDC_BOARD */
...
static int msm_fb_resume(struct platform_device *pdev)
{
	...
	pr_info_klog("[====]%s:\n", __func__);
	msm_fb_suspend_resume=1;
	msm_fb_eventlog_rec(EEVENT_MSM_FB_RESUME, 0);
#endif	/* CONFIG_SHLCDC_BOARD */
...
static void msmfb_early_suspend(struct early_suspend *h)
{
	...
	msm_fb_early_suspend_resume=1;
	msm_fb_eventlog_rec(EEVENT_MSM_FB_EARLY_SUSPEND, 0);
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[====]%s:start\n", __func__);
	msm_fb_early_suspend_resume=0;
	msm_fb_eventlog_rec(EEVENT_MSM_FB_EARLY_RESUME, 0);
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_ANDROID_ENGINEERING
module_param(unset_bl_level, int, 0400);
module_param(bl_updated, int, 0400);
module_param(bl_level_old, int, 0400);
#endif
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[====]%s:\n", __func__);
	msm_fb_eventlog_rec(EEVENT_MSM_FB_SET_BACKLIGHT, bkl_lvl);
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[====]%s:\n", __func__);
	msm_fb_eventlog_rec(EEVENT_MSM_FB_BLANK_SUB, blank_mode);
#endif	/* CONFIG_SHLCDC_BOARD */
	if (!op_enable)
		return -EPERM;
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_pandisp("[====]%s:\n", __func__);
	msm_fb_busylog_rec(EEVENT_MSM_FB_PAN_DISPLAY, 0);
	if(msm_fb_pandisp) msm_fb_eventlog_rec(EEVENT_MSM_FB_PAN_DISPLAY, 0);
#endif	/* CONFIG_SHLCDC_BOARD */
	if (info->node != 0 || mfd->cont_splash_done)	/* primary */
...
static int msmfb_overlay_play_w(struct fb_info *info, unsigned long *argp)
{
	...
	ret = mdp4_overlay_play_w(info, &req);
			...
			unsigned long arg)
{
...
#ifdef	CONFIG_SHLCDC_BOARD
	msm_fb_last_ioctl_cmd=cmd;
	msm_fb_eventlog_rec(EEVENT_MSM_FB_IOCTL, cmd);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog_overlay("[====]%s:MSMFB_OVERLAY_GET\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog_overlay("[====]%s:MSMFB_OVERLAY_SET\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		down(&msm_fb_ioctl_ppp_sem);
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_OVERLAY_UNSET\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		down(&msm_fb_ioctl_ppp_sem);
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog_overlay("[====]%s:MSMFB_OVERLAY_PLAY\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		down(&msm_fb_ioctl_ppp_sem);
	...
	case MSMFB_OVERLAY_PLAY_W:
		pr_info_klog_overlay("[====]%s:MSMFB_OVERLAY_PLAY_W\n", __func__);
		down(&msm_fb_ioctl_ppp_sem);
		ret = msmfb_overlay_play_w(info, argp);
		up(&msm_fb_ioctl_ppp_sem);
		break;
	case MSMFB_PAN_ENABLE:
		pr_info_klog("[====]%s:MSMFB_PAN_ENABLE\n", __func__);
		down(&msm_fb_ioctl_ppp_sem);
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_OVERLAY_PLAY_ENABLE\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		down(&msm_fb_ioctl_ppp_sem);
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_OVERLAY_PLAY_WAIT\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		down(&msm_fb_ioctl_ppp_sem);
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_OVERLAY_BLT\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		down(&msm_fb_ioctl_ppp_sem);
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_OVERLAY_BLT_OFFSET\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		down(&msm_fb_ioctl_ppp_sem);
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_OVERLAY_3D\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		down(&msm_fb_ioctl_ppp_sem);
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_MIXER_INFO\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		down(&msm_fb_ioctl_ppp_sem);
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_WRITEBACK_INIT\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		ret = msmfb_overlay_ioctl_writeback_init(info);
		break;
	case MSMFB_WRITEBACK_START:
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_WRITEBACK_START\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_WRITEBACK_STOP\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_WRITEBACK_QUEUE_BUFFER\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_WRITEBACK_DEQUEUE_BUFFER\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_WRITEBACK_TERMINATE\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_SET_CCS_MATRIX\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
#ifndef CONFIG_FB_MSM_MDP40
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_GET_CCS_MATRIX\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
#ifndef CONFIG_FB_MSM_MDP40
		ret = copy_from_user(&ccs_matrix, argp, sizeof(ccs_matrix)) ;
		if (ret) {
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_SUSPEND_SW_REFRESHER\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		if (!mfd->panel_power_on)
			return -EPERM;
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_RESUME_SW_REFRESHER\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		if (!mfd->panel_power_on)
			return -EPERM;
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_HISTOGRAM\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		if (!mfd->panel_power_on)
			return -EPERM;
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_HISTOGRAM_START\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		if (!mfd->panel_power_on)
			return -EPERM;
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_HISTOGRAM_STOP\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
		if (!mfd->do_histogram)
			return -ENODEV;
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_GET_PAGE_PROTECTION\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_NOTIFY_UPDATE\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[====]%s:MSMFB_SET_PAGE_PROTECTION\n", __func__);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[====]%s:\n", __func__);
	msm_fb_eventlog_rec(EEVENT_MSM_FB_GET_WRITEBACK_FB, 0);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[====]%s:\n", __func__);
	msm_fb_eventlog_rec(EEVENT_MSM_FB_WRITEBACK_START, 0);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[====]%s:\n", __func__);
	msm_fb_eventlog_rec(EEVENT_MSM_FB_WRITEBACK_QUEUE_BUFFER, 0);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[====]%s:\n", __func__);
	msm_fb_eventlog_rec(EEVENT_MSM_FB_WRITEBACK_DEQUEUE_BUFFER, 0);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[====]%s:\n", __func__);
	msm_fb_eventlog_rec(EEVENT_MSM_FB_WRITEBACK_STOP, 0);
#endif	/*CONFIG_SHLCDC_BOARD*/
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[====]%s:\n", __func__);
	msm_fb_eventlog_rec(EEVENT_MSM_FB_WRITEBACK_INIT, 0);
#endif	/*CONFIG_SHLCDC_BOARD*/
	return mdp4_writeback_init(info);
}
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[====]%s:\n", __func__);
	msm_fb_eventlog_rec(EEVENT_MSM_FB_WRITEBACK_TERMINATE, 0);
#endif	/*CONFIG_SHLCDC_BOARD*/
	return mdp4_writeback_terminate(info);
}
...
int __init msm_fb_init(void)
{
	...
	if(strncmp(SH_BUILD_ID, "S", 1) != 0){
		kmsg_dump_register(&kmsg_dumper_msm_fb);
		modem_register_notifier(&nb_msm_fb);
	}
#endif	/* CONFIG_SHLCDC_BOARD */
