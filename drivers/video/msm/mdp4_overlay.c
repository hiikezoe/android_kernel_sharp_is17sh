...
// Quick Patch for Link error : begin
#ifdef	CONFIG_SHLCDC_BOARD
#ifndef	CONFIG_FB_MSM_EXT_INTERFACE_COMMON
...
#endif
#endif
// Quick Patch for Link error : end

#ifdef	CONFIG_SHLCDC_BOARD
static int mdp4_overlay_klog_overlay=0;
static int mdp4_overlay_klog=0;
#ifdef CONFIG_ANDROID_ENGINEERING
module_param(mdp4_overlay_klog_overlay, int, 0600);
module_param(mdp4_overlay_klog, int, 0600);
#endif
#define pr_info_klog_overlay(fmt, arg...)	\
	if(mdp4_overlay_klog_overlay>0) pr_info(fmt, ## arg);
#define pr_info_klog(fmt, arg...)	\
	if(mdp4_overlay_klog>0) pr_info(fmt, ## arg);

enum enum_mdp4_overlay_event {
	EEVENT_MDP4_OVERLAY_MIXIER_PLAY,
	EEVENT_MDP4_OVERLAY_GET,
	EEVENT_MDP4_OVERLAY_SET,
	EEVENT_MDP4_OVERLAY_UNSET,
	EEVENT_MDP4_OVERLAY_PLAY_WAIT,
	EEVENT_MDP4_OVERLAY_PLAY,
	EEVENT_MDP4_OVERLAY_PLAY_W,
	EEVENT_MDP4_OVERLAY_MDP_RESET,
};


#define	EVENTLOG_MDP4_OVERLAY	0x400
static void mdp4_overlay_eventlog_rec(unsigned long event, unsigned long param)
{
	shlcdc_eventlog_rec(SHLCDC_EVENTLOG_FB + EVENTLOG_MDP4_OVERLAY + event, param);
}

static void mdp4_overlay_busylog_rec(unsigned long event, unsigned long param)
{
	shlcdc_busylog_rec(SHLCDC_EVENTLOG_FB + EVENTLOG_MDP4_OVERLAY + event, param);
}
...
#ifdef	CONFIG_SHLCDC_BOARD
struct mdp4_overlay_pipe *mdp4_overlay_ndx2pipe_pipeonly(int ndx)
{
	struct mdp4_overlay_pipe *pipe;
...
#ifdef CONFIG_SHLCDC_BOARD
	pr_info_klog("[++++]%s:id=%d, z_order=%d, mixer=%d, pipe->pipe_ndx=%d, ctrl->stage[MDP4_MIXER0][MDP4_MIXER_STAGE_BASE]->pipe_ndx=%d\n", __func__, id, z_order, mixer, pipe->pipe_ndx, ctrl->stage[MDP4_MIXER0][MDP4_MIXER_STAGE_BASE]->pipe_ndx);
#endif
	return -EPERM;
}
#endif
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog_overlay("[++++]%s\n", __func__);
	mdp4_overlay_busylog_rec(EEVENT_MDP4_OVERLAY_GET, 0);
#endif	/* CONFIG_SHLCDC_BOARD */
	pipe = mdp4_overlay_ndx2pipe(req->id);
	if (pipe == NULL)
		return -ENODEV;
...
#define OVERLAY_QVGA_SIZE	0x012C00
...
{
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog("[++++]%s:perf_level [update] %d\n", __func__, perf_level);
#endif	/* CONFIG_SHLCDC_BOARD */
	new_perf_level = perf_level;
}

#ifdef	CONFIG_SHLCDC_BOARD
int mdp4_overlay_running_perf_level = 0;
#ifdef	CONFIG_ANDROID_ENGINEERING
module_param(mdp4_overlay_running_perf_level, int, 0400);
#endif
#endif
...
#ifdef	CONFIG_SHLCDC_BOARD
		pr_info_klog("[++++]%s:perf_level [change] %d->%d\n", __func__, old_perf_level, new_perf_level);
#endif	/* CONFIG_SHLCDC_BOARD */
		mdp_set_core_clk(cur_perf_level);
		old_perf_level = cur_perf_level;
#ifdef	CONFIG_SHLCDC_BOARD
		mdp4_overlay_running_perf_level = cur_perf_level;
#endif
...
int mdp4_overlay_set(struct fb_info *info, struct mdp_overlay *req)
{
...
#ifdef	CONFIG_SHLCDC_BOARD
	mdp4_overlay_busylog_rec(EEVENT_MDP4_OVERLAY_SET, 0);
#endif
	if (mfd == NULL) {
		pr_err("%s: mfd == NULL, -ENODEV\n", __func__);
		return -ENODEV;
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog_overlay("[++++]%s:pdest=%d, src.format=%d, src_width=%d, src_height=%d, src_rect=[%d,%d,%d,%d], dst_rect=[%d,%d,%d,%d]\n", __func__, ((struct msm_panel_info *)&mfd->panel_info)->pdest, (int)req->src.format, (int)req->src.width, (int)req->src.height, (int)req->src_rect.x, (int)req->src_rect.y, (int)req->src_rect.w, (int)req->src_rect.h, (int)req->dst_rect.x, (int)req->dst_rect.y, (int)req->dst_rect.w, (int)req->dst_rect.h);
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog_overlay("[++++]%s:mdp4_overlay_get_perf_level() result=%d\n", __func__, perf_level);
#endif	/* CONFIG_SHLCDC_BOARD */
	...
	if(req->id == ctrl->stage[MDP4_MIXER0][MDP4_MIXER_STAGE_BASE]->pipe_ndx){
		if ((mfd->panel_info.xres != req->dst_rect.w)
			...
			pr_err("[++++]%s:panel_info.xres=%d, panel_info.yres=%d, dst_rect=[%d,%d,%d,%d] <return -ERANGE>\n", __func__, mfd->panel_info.xres, mfd->panel_info.yres, (int)req->dst_rect.x, (int)req->dst_rect.y, (int)req->dst_rect.w, (int)req->dst_rect.h);
	...
	if ( ((req->src_rect.h > req->dst_rect.h) || (req->src_rect.w > req->dst_rect.w)) &&
		 (mdp4_overlay_format2type(req->src.format) == OVERLAY_TYPE_VIDEO) ) {
			pr_info_klog("[++++]%s:perf_level=OVERLAY_PERF_LEVEL1(reduce)\n", __func__);
			perf_level = OVERLAY_PERF_LEVEL1;
	}

	if ( ((req->src_rect.w > req->src_rect.h) ^ (req->dst_rect.w > req->dst_rect.h)) &&
		 (mdp4_overlay_format2type(req->src.format) == OVERLAY_TYPE_VIDEO) ) {
			pr_info_klog("[++++]%s:perf_level=OVERLAY_PERF_LEVEL1(portrait<-->landscape)\n", __func__);
			perf_level = OVERLAY_PERF_LEVEL1;
	}

#if !defined(CONFIG_MACH_LYNX_DC60)
#if defined(CONFIG_MACH_NNK)
	if ((req->src.width * req->src.height <= OVERLAY_QVGA_SIZE) &&
#else
	if ((req->src.width * req->src.height < OVERLAY_VGA_SIZE) &&
#endif
         (req->src.width > req->src.height) &&
		 (mdp4_overlay_format2type(req->src.format) == OVERLAY_TYPE_VIDEO) ) {
			pr_info_klog("[++++]%s:perf_level=OVERLAY_PERF_LEVEL2(DTV correction)\n", __func__);
			if(perf_level > OVERLAY_PERF_LEVEL2)
				perf_level = OVERLAY_PERF_LEVEL2;
			if(clk_get_rate(clk_get(NULL, "ebi1_mddi_clk")) < 192000000)
			{
				pr_info_klog("[++++]%s:ebi1_mddk_clk -> %lu\n", __func__, (unsigned long)192000000);
				clk_set_rate(clk_get(NULL, "ebi1_mddi_clk"), 192000000);
			}
	}
#endif
...
#ifdef CONFIG_SHLCDC_BOARD
	if((pipe->pipe_ndx == 3) || (pipe->pipe_ndx == 4)){
...
int mdp4_overlay_unset(struct fb_info *info, int ndx)
{
...
#ifdef	CONFIG_SHLCDC_BOARD
	mdp4_overlay_busylog_rec(EEVENT_MDP4_OVERLAY_UNSET, 0);
	pr_info_klog_overlay("[++++]%s:ndx=%d\n", __func__, ndx);
#endif	/* CONFIG_SHLCDC_BOARD */
	if (mfd == NULL)
		return -ENODEV;
	...
	if (pipe->mixer_num == MDP4_MIXER1) {	/* mixer1, DTV, ATV */
		if (ctrl->panel_mode & MDP4_PANEL_DTV) {
			if (mfd->panel_power_on) {
...
#if !defined(CONFIG_MACH_LYNX_DC60)
	if(clk_get_rate(clk_get(NULL, "ebi1_mddi_clk")) > 65000000)
	{
		pr_info_klog("[++++]%s:ebi1_mddk_clk -> %lu\n", __func__, (unsigned long)65000000);
		clk_set_rate(clk_get(NULL, "ebi1_mddi_clk"), 65000000);
	}
#endif /*CONFIG_MACH_DECKARD_AF33*/
...
#ifdef	CONFIG_SHLCDC_BOARD
	mdp4_overlay_busylog_rec(EEVENT_MDP4_OVERLAY_PLAY_WAIT, 0);
	pr_info_klog_overlay("[++++]%s\n", __func__);
#endif	/* CONFIG_SHLCDC_BOARD */
	if (mfd == NULL)
		return -ENODEV;
...
#ifdef	CONFIG_SHLCDC_BOARD
	mdp4_overlay_busylog_rec(EEVENT_MDP4_OVERLAY_PLAY, 0);
	pr_info_klog_overlay("[++++]%s\n", __func__);
#endif	/* CONFIG_SHLCDC_BOARD */
	if (mfd == NULL)
		return -ENODEV;
...
#ifdef	CONFIG_SHLCDC_BOARD
{
	extern int mdp4_util_csc_convert_setup_exec;
	if(mdp4_util_csc_convert_setup_exec){
		mdp4_vg_csc_setup(pipe->pipe_num-OVERLAY_PIPE_VG1);
		mdp4_util_csc_convert_setup_exec=0;
	}
}
#endif

#ifdef	CONFIG_SHLCDC_BOARD
	pr_info_klog_overlay("[++++]%s:src_format=%d, src_width=%d, src_height=%d, src=[%d,%d,%d,%d], dst=[%d,%d,%d,%d]\n", __func__, pipe->src_format, pipe->src_width, pipe->src_height, pipe->src_x, pipe->src_y, pipe->src_w, pipe->src_h, pipe->dst_x, pipe->dst_y, pipe->dst_w, pipe->dst_h);
#endif
	if (mutex_lock_interruptible(&mfd->dma->ov_mutex))
		return -EINTR;
...
int mdp4_overlay_play_w(struct fb_info *info, struct msmfb_overlay_data_w *req)
{
	...
	ulong start, addr;
	ulong len = 0;
	struct file *srcp0_file1 = NULL;
	struct file *srcp1_file1 = NULL, *srcp2_file1 = NULL;
	struct ion_handle *srcp0_ihdl1 = NULL;
	struct ion_handle *srcp1_ihdl1 = NULL, *srcp2_ihdl1 = NULL;

	struct file *srcp0_file2 = NULL;
	struct file *srcp1_file2 = NULL, *srcp2_file2 = NULL;
	struct ion_handle *srcp0_ihdl2 = NULL;
	struct ion_handle *srcp1_ihdl2 = NULL, *srcp2_ihdl2 = NULL;
	int ps0_need, p_need;
...
#ifdef	CONFIG_SHLCDC_BOARD
	mdp4_overlay_busylog_rec(EEVENT_MDP4_OVERLAY_PLAY_W, 0);
	pr_info_klog_overlay("[++++]%s\n", __func__);
#endif	/* CONFIG_SHLCDC_BOARD */
	if (mfd == NULL)
		return -ENODEV;
	...
	if (pipe1 == NULL) {
		pr_err("%s: req_id1=%d Error\n", __func__, req->id1);
		return -ENODEV;
	}

	pipe2 = mdp4_overlay_ndx2pipe(req->id2);
	if (pipe2 == NULL) {
		pr_err("%s: req_id2=%d Error\n", __func__, req->id2);
		return -ENODEV;
	}
	...
	img = &req->data1;
	get_img(img, info, &start, &len, &srcp0_file1, &ps0_need, &srcp0_ihdl1);
	if (len == 0) {
		mutex_unlock(&mfd->dma->ov_mutex);
		pr_err("%s: pmem Error\n", __func__);
		ret = -1;
	...
	if (pipe1->fetch_plane == OVERLAY_PLANE_PSEUDO_PLANAR) {
		if (overlay_version > 0) {
			img = &req->plane1_data1;
			get_img(img, info, &start, &len, &srcp1_file1,
				&p_need, &srcp1_ihdl1);
			if (len == 0) {
				mutex_unlock(&mfd->dma->ov_mutex);
				pr_err("%s: Error to get plane1\n", __func__);
			...
			pipe1->srcp1_addr = start + img->offset;
		} else if (pipe1->frame_format ==
				MDP4_FRAME_FORMAT_VIDEO_SUPERTILE) {
			struct tile_desc tile;
			...
			pipe1->srcp1_addr = addr + (pipe1->src_width *
						pipe1->src_height);
		}
		pipe1->srcp0_ystride = pipe1->src_width;
		if ((pipe1->src_format == MDP_Y_CRCB_H1V1) ||
			(pipe1->src_format == MDP_Y_CBCR_H1V1)) {
			if (pipe1->src_width > YUV_444_MAX_WIDTH)
				pipe1->srcp1_ystride = pipe1->src_width << 2;
			else
				pipe1->srcp1_ystride = pipe1->src_width << 1;
		} else
		...
		if (overlay_version > 0) {
			...
			img = &req->plane2_data1;
			get_img(img, info, &start, &len, &srcp2_file1,
				&p_need, &srcp2_ihdl1);
			if (len == 0) {
				mutex_unlock(&mfd->dma->ov_mutex);
				pr_err("%s: Error to get plane2\n", __func__);
			...
			pipe1->srcp2_addr = start + img->offset;
		} else {
			if (pipe1->src_format == MDP_Y_CR_CB_GH2V2) {
				addr += (ALIGN(pipe1->src_width, 16) *
					pipe1->src_height);
				pipe1->srcp1_addr = addr;
				addr += ((ALIGN((pipe1->src_width / 2), 16)) *
					(pipe1->src_height / 2));
				pipe1->srcp2_addr = addr;
			} else {
				addr += (pipe1->src_width * pipe1->src_height);
				pipe1->srcp1_addr = addr;
				addr += ((pipe1->src_width / 2) *
					(pipe1->src_height / 2));
				pipe1->srcp2_addr = addr;
			}
		}
		/* mdp planar format expects Cb in srcp1 and Cr in p2 */
		if ((pipe1->src_format == MDP_Y_CR_CB_H2V2) ||
			(pipe1->src_format == MDP_Y_CR_CB_GH2V2))
			swap(pipe1->srcp1_addr, pipe1->srcp2_addr);

		if (pipe1->src_format == MDP_Y_CR_CB_GH2V2) {
			pipe1->srcp0_ystride = ALIGN(pipe1->src_width, 16);
			pipe1->srcp1_ystride = ALIGN(pipe1->src_width / 2, 16);
			pipe1->srcp2_ystride = ALIGN(pipe1->src_width / 2, 16);
		} else {
			pipe1->srcp0_ystride = pipe1->src_width;
			pipe1->srcp1_ystride = pipe1->src_width / 2;
	...
	if( req->data1.memory_id != req->data2.memory_id ){
		img = &req->data2;
		get_img(img, info, &start, &len, &srcp0_file2, &p_need, &srcp0_ihdl2);
		if (len == 0) {
			mutex_unlock(&mfd->dma->ov_mutex);
			pr_err("%s: pmem Error\n", __func__);
			ret = -1;
	...
	if (pipe2->fetch_plane == OVERLAY_PLANE_PSEUDO_PLANAR) {
		if (overlay_version > 0) {
			img = &req->plane1_data2;
			get_img(img, info, &start, &len, &srcp1_file2,
				&p_need, &srcp1_ihdl2);
			if (len == 0) {
				mutex_unlock(&mfd->dma->ov_mutex);
				pr_err("%s: Error to get plane1\n", __func__);
			...
			pipe2->srcp1_addr = start + img->offset;
		} else if (pipe2->frame_format ==
				MDP4_FRAME_FORMAT_VIDEO_SUPERTILE) {
			struct tile_desc tile;
			...
			pipe2->srcp1_addr = addr + (pipe2->src_width *
						pipe2->src_height);
		}
		pipe2->srcp0_ystride = pipe2->src_width;
		if ((pipe2->src_format == MDP_Y_CRCB_H1V1) ||
			(pipe2->src_format == MDP_Y_CBCR_H1V1)) {
			if (pipe2->src_width > YUV_444_MAX_WIDTH)
				pipe2->srcp1_ystride = pipe2->src_width << 2;
			else
				pipe2->srcp1_ystride = pipe2->src_width << 1;
		} else
		...
		if (overlay_version > 0) {
			...
			img = &req->plane2_data2;
			get_img(img, info, &start, &len, &srcp2_file2,
				&p_need, &srcp2_ihdl2);
			if (len == 0) {
				mutex_unlock(&mfd->dma->ov_mutex);
				pr_err("%s: Error to get plane2\n", __func__);
			...
			pipe2->srcp2_addr = start + img->offset;
		} else {
			if (pipe2->src_format == MDP_Y_CR_CB_GH2V2) {
				addr += (ALIGN(pipe2->src_width, 16) *
					pipe2->src_height);
				pipe2->srcp1_addr = addr;
				addr += ((ALIGN((pipe2->src_width / 2), 16)) *
					(pipe2->src_height / 2));
				pipe2->srcp2_addr = addr;
			} else {
				addr += (pipe2->src_width * pipe2->src_height);
				pipe2->srcp1_addr = addr;
				addr += ((pipe2->src_width / 2) *
					(pipe2->src_height / 2));
				pipe2->srcp2_addr = addr;
			}
		}
		/* mdp planar format expects Cb in srcp1 and Cr in p2 */
		if ((pipe2->src_format == MDP_Y_CR_CB_H2V2) ||
			(pipe2->src_format == MDP_Y_CR_CB_GH2V2))
			swap(pipe2->srcp1_addr, pipe2->srcp2_addr);

		if (pipe2->src_format == MDP_Y_CR_CB_GH2V2) {
			pipe2->srcp0_ystride = ALIGN(pipe2->src_width, 16);
			pipe2->srcp1_ystride = ALIGN(pipe2->src_width / 2, 16);
			pipe2->srcp2_ystride = ALIGN(pipe2->src_width / 2, 16);
		} else {
			pipe2->srcp0_ystride = pipe2->src_width;
			pipe2->srcp1_ystride = pipe2->src_width / 2;
	...
	if (pipe1->pipe_num >= OVERLAY_PIPE_VG1)
		...
		if (pipe1->flags & MDP_SHARPENING) {
			pr_warn(
			"%s: Sharpening/Smoothing not supported on RGB pipe\n",
								     __func__);
			pipe1->flags &= ~MDP_SHARPENING;
		}
	...
	if (pipe2->pipe_num >= OVERLAY_PIPE_VG1)
		...
		if (pipe2->flags & MDP_SHARPENING) {
			pr_warn(
			"%s: Sharpening/Smoothing not supported on RGB pipe\n",
								     __func__);
			pipe2->flags &= ~MDP_SHARPENING;
		}
	...
	/* write out DPP HSIC registers */
	if (pipe1->flags & MDP_DPP_HSIC)
		mdp4_hsic_update(pipe1);
	if (pipe2->flags & MDP_DPP_HSIC)
		mdp4_hsic_update(pipe2);
...
#ifdef CONFIG_ANDROID_PMEM
	if (srcp0_file1)
		put_pmem_file(srcp0_file1);
	if (srcp1_file1)
		put_pmem_file(srcp1_file1);
	if (srcp2_file1)
		put_pmem_file(srcp2_file1);
	if (srcp0_file2)
		put_pmem_file(srcp0_file2);
	if (srcp1_file2)
		put_pmem_file(srcp1_file2);
	if (srcp2_file2)
		put_pmem_file(srcp2_file2);
#endif
#ifdef CONFIG_MSM_MULTIMEDIA_USE_ION
	if (!IS_ERR_OR_NULL(srcp0_ihdl1))
		ion_free(mfd->client, srcp0_ihdl1);
	if (!IS_ERR_OR_NULL(srcp1_ihdl1))
		ion_free(mfd->client, srcp1_ihdl1);
	if (!IS_ERR_OR_NULL(srcp2_ihdl1))
		ion_free(mfd->client, srcp2_ihdl1);
	if (!IS_ERR_OR_NULL(srcp0_ihdl2))
		ion_free(mfd->client, srcp0_ihdl2);
	if (!IS_ERR_OR_NULL(srcp1_ihdl2))
		ion_free(mfd->client, srcp1_ihdl2);
	if (!IS_ERR_OR_NULL(srcp2_ihdl2))
		ion_free(mfd->client, srcp2_ihdl2);
#endif
	return ret;
}
#endif /*CONFIG_SHLCDC_BOARD*/
...
void mdp4_overlay_mdp_reset(unsigned char reset)
{
	...
	mdp4_overlay_eventlog_rec(EEVENT_MDP4_OVERLAY_MDP_RESET, reset);
	switch(reset){
		case 1:	/* mixer0 */
			mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
			mdp4_sw_reset(0x01);
