...
/* GPIO mapping for RTS CTS UART Tx/Rx */
#define UART_PORT_RTS           134     /* UART1_RTS_N  */
#define UART_PORT_CTS           135     /* UART1_CTS_N  */
#define UART_PORT_RX            136     /* UART1_RXD    */
#define UART_PORT_TX            137     /* UART1_TXD    */
#define BT_WAKE_OUT              86     /* BT_WAKEOUT_N */
#define HOST_WAKE_IN            142     /* BT_WAKE_IN   */

/* Port Configuration Table */
static unsigned uart_gpio_config[] = {
    /*        gpio,          func, dir,             pull,             drvstr */
#if defined(CONFIG_MACH_DECKARD_AF20) || defined(CONFIG_MACH_DECKARD_AF21) || defined(CONFIG_MACH_DECKARD_AF30) || defined(CONFIG_MACH_DECKARD_AF33) || defined(CONFIG_MACH_DECKARD_AS35) || defined(CONFIG_MACH_DECKARD_AS50)
    GPIO_CFG( UART_PORT_RTS, 1,    GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA),
    GPIO_CFG( UART_PORT_CTS, 1,    GPIO_CFG_INPUT,  GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
    GPIO_CFG( UART_PORT_RX,  1,    GPIO_CFG_INPUT,  GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
    GPIO_CFG( UART_PORT_TX,  1,    GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA),
#elif defined(CONFIG_MACH_DECKARD_AS36) || defined(CONFIG_MACH_DECKARD_AS46)
    GPIO_CFG( UART_PORT_RTS, 1,    GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_10MA),
    GPIO_CFG( UART_PORT_CTS, 1,    GPIO_CFG_INPUT,  GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
    GPIO_CFG( UART_PORT_RX,  1,    GPIO_CFG_INPUT,  GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
    GPIO_CFG( UART_PORT_TX,  1,    GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_10MA),
#else
    GPIO_CFG( UART_PORT_RTS, 1,    GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_6MA),
    GPIO_CFG( UART_PORT_CTS, 1,    GPIO_CFG_INPUT,  GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
    GPIO_CFG( UART_PORT_RX,  1,    GPIO_CFG_INPUT,  GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
...
enum msm_hs_serial_clk_state {
    MSM_HS_SERIAL_CLK_OFF,
    MSM_HS_SERIAL_CLK_ON,
};

enum msm_hs_serial_gpio_state {
    MSM_HS_SERIAL_GPIO_LOW,
    MSM_HS_SERIAL_GPIO_HIGH,
};

enum msm_hs_serial_irq_state {
    MSM_HS_SERIAL_IRQ_ENABLE,
    MSM_HS_SERIAL_IRQ_DISABLE,
};
...
#ifdef CONFIG_SERIAL_BLUETOOTH
	int irq_status;                  /* enable_irq/disable_irq control flag */
#endif /* CONFIG_SERIAL_BLUETOOTH */
};
...
#else  /* Not CONFIG_SERIAL_BLUETOOTH */
	wake_lock(&msm_uport->dma_wake_lock);
#endif /* CONFIG_SERIAL_BLUETOOTH */
...
#ifndef CONFIG_SERIAL_BLUETOOTH
	wake_lock_timeout(&msm_uport->rx.wake_lock, HZ / 2);
#endif /* Not CONFIG_SERIAL_BLUETOOTH */
...
static int msm_hs_check_clock_off_locked(struct uart_port *uport)
{
	...
	if (gpio_get_value(UART_PORT_RTS) == MSM_HS_SERIAL_GPIO_LOW) {
		gpio_set_value( UART_PORT_RTS, MSM_HS_SERIAL_GPIO_HIGH );		/* UART1_RTS_N HIGH */
		/* RTS Port Configuration */
		result = gpio_tlmm_config( uart_gpio_config[GPIO_CONFIG_RTS], GPIO_CFG_DISABLE );
		if (result) {
			printk(KERN_ERR  "gpio_tlmm_config(GPIO_CONFIG_RTS)<-%#x : %d\n", uart_gpio_config[GPIO_CONFIG_RTS], result);
			return -ENXIO;
		}
	}

	if (msm_uport->irq_status == MSM_HS_SERIAL_IRQ_ENABLE) {
		disable_irq_nosync(msm_uport->wakeup.irq);
		msm_uport->irq_status = MSM_HS_SERIAL_IRQ_DISABLE;
	}
...
#ifdef CONFIG_SERIAL_BLUETOOTH
		if (msm_uport->irq_status == MSM_HS_SERIAL_IRQ_DISABLE) {
			enable_irq(msm_uport->wakeup.irq);
			msm_uport->irq_status = MSM_HS_SERIAL_IRQ_ENABLE;
		}
#endif /* CONFIG_SERIAL_BLUETOOTH */
		return -1;
	}
...
void msm_hs_request_clock_off(struct uart_port *uport) {
	...
	if (clk_state == MSM_HS_CLK_ON) {
		...
		spin_lock_irqsave(&uport->lock, flags);
		gpio_set_value( BT_WAKE_OUT, MSM_HS_SERIAL_GPIO_HIGH );			/* BT_WAKEOUT_N HIGH(SLEEP) */
		spin_unlock_irqrestore(&uport->lock, flags);

		spin_lock_irqsave(&uport->lock, flags);
		if (gpio_get_value(HOST_WAKE_IN) == MSM_HS_SERIAL_GPIO_HIGH) {
...
/* Request to uart clock state */
void msm_hs_request_get_clock_state(struct uart_port *uport, unsigned int *state) {
	struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);

	if (msm_uport->clk_state == MSM_HS_CLK_ON) {
		*state = MSM_HS_SERIAL_CLK_ON;
	} else {
		*state = MSM_HS_SERIAL_CLK_OFF;
	}
}
EXPORT_SYMBOL(msm_hs_request_get_clock_state);
#endif /* CONFIG_SERIAL_BLUETOOTH */
...
#ifdef CONFIG_SERIAL_BLUETOOTH
		/* RTS Port Configuration */
		result = gpio_tlmm_config( uart_gpio_config[GPIO_CONFIG_RTS], GPIO_CFG_ENABLE );
		if (result) {
			printk(KERN_ERR  "gpio_tlmm_config(GPIO_CONFIG_RTS)<-%#x : %d\n", uart_gpio_config[GPIO_CONFIG_RTS], result);
		}
		gpio_set_value( UART_PORT_RTS, MSM_HS_SERIAL_GPIO_LOW );		/* UART1_RTS_N LOW */
#endif /* CONFIG_SERIAL_BLUETOOTH */
...
void msm_hs_request_clock_on(struct uart_port *uport) {
	...
	spin_lock_irqsave(&uport->lock, flags);
#ifdef CONFIG_SERIAL_BLUETOOTH
	gpio_set_value( BT_WAKE_OUT, MSM_HS_SERIAL_GPIO_LOW );				/* BT_WAKEOUT_N LOW(WAKE) */
#endif /* CONFIG_SERIAL_BLUETOOTH */
...
static irqreturn_t msm_hs_wakeup_isr(int irq, void *dev)
{
	...
	spin_lock_irqsave(&uport->lock, flags);
	host_wake_state = gpio_get_value(HOST_WAKE_IN);
	bt_wake_state = gpio_get_value(BT_WAKE_OUT);
	clk_state = msm_uport->clk_state;
	spin_unlock_irqrestore(&uport->lock, flags);

	if (host_wake_state == MSM_HS_SERIAL_GPIO_LOW) {					/* HOST_WAKE FALLING */
		if (clk_state == MSM_HS_CLK_OFF) {
			spin_lock_irqsave(&uport->lock, flags);
			...
			printk(KERN_INFO  "Info HOST_WAKE FALLING  clk_state:%d\n", clk_state);
		}
	} else {															/* HOST_WAKE RISING */
		if (clk_state == MSM_HS_CLK_ON) {
			if (bt_wake_state == MSM_HS_SERIAL_GPIO_HIGH) {
				msm_hs_request_clock_off(&msm_uport->uport);
			} else {
				printk(KERN_INFO  "Info HOST_WAKE RISING  clk_state:%d\n", clk_state);
			}
		} else {
			printk(KERN_ERR  "Error HOST_WAKE RISING  clk_state:%d\n", clk_state);
		}
	}
	return IRQ_HANDLED;
#else  /* Not CONFIG_SERIAL_BLUETOOTH */
