...
extern int msmsdcc_set_transfer_mode(struct mmc_host *mmc);
#endif
...
static int mmc_blk_issue_rw_rq(struct mmc_queue *mq, struct request *req)
{
	...
	int err_retry_max = 3;
	int err_retry = 0;
	int err_init_retry_max = 3;
	int err_init_retry = 0;
...
cmd_err_retry:
	...
	do {
		...
		if (mmc_card_mmc(card)) {
			...
			if (mmc_check_protect_part(req, &blocks, &protect_err)
									!= 0) {

				printk(KERN_WARNING "%s:%u: protect status %d "
					"sector %u, nr %u, blocks %d\n",
					req->rq_disk->disk_name,
					(unsigned)(rq_data_dir(req)),
					protect_err,
					(unsigned)(blk_rq_pos(req)),
					(unsigned)(blk_rq_sectors(req)),
					blocks);

				if ((brq.data.blocks <= blocks)
				&& (protect_err != 0)) {
					spin_lock_irq(&md->lock);
					while (ret)
				...
				spin_lock_irq(&md->lock);
				ret = __blk_end_request(req, protect_err,
							blocks * 512);
				spin_unlock_irq(&md->lock);
				continue;
...
#ifdef CONFIG_ERR_RETRY_EMMC_CUST_SH
			if (rq_data_dir(req) == READ && err_init_retry >= err_init_retry_max) {
#else
			if (rq_data_dir(req) == READ) {
#endif
 ...
 cmd_err:
	...
	if (err_init_retry < err_init_retry_max) {
		err_init_retry++;
		
		msmsdcc_set_transfer_mode(card->host);
		printk(KERN_WARNING "%s: recovery retry(%d)\n",
			req->rq_disk->disk_name, err_init_retry);
