...
#endif /* CONFIG_MMC_SDIO_CUST_SH || CONFIG_IDLECTRL_EMMC_CUST_SH */

#ifdef CONFIG_IDLECTRL_EMMC_CUST_SH
struct mmc_host *msmsdcc_idle_mmc1 = NULL;
unsigned int  msmsdcc_state_suspend_mmc1 = 0;
unsigned int  msmsdcc_state_idle_mmc1 = 0;
unsigned int  msmsdcc_state_rt_suspend_mmc1 = 0;
#endif /* CONFIG_IDLECTRL_EMMC_CUST_SH */

#ifdef CONFIG_MMC_SDIO_CUST_SH
struct mmc_host *msmsdcc_idle_mmc0 = NULL;
...
int msmsdcc_set_transfer_mode(struct mmc_host *mmc)
{
	int ret = 0;

	ret = mmc_power_save_host(mmc);
	ret = mmc_power_restore_host(mmc);

	if (ret)
		printk(KERN_ERR "%s recovery error!!\n", __func__);
	else
	  ...
	  if (strncmp(mmc_hostname(mmc),"mmc1",4) != 0) {
		...
		if (!rc) {
			...
			if (mmc->card && (mmc->card->type == MMC_TYPE_SDIO) &&
				...
				mmc->ios.clock = 0;
				msmsdcc_state_rt_suspend_mmc1 = 1;
				mmc->ops->set_ios(host->mmc, &host->mmc->ios);
			}
		#endif /* CONFIG_IDLECTRL_EMMC_CUST_SH */
		}
	...
	if (mmc) {
		if (mmc->card && mmc->card->type == MMC_TYPE_SDIO) {
		  ...
		  if( strncmp(mmc_hostname(mmc),"mmc0",4) != 0){
			...
			mmc->ios.clock = host->clk_rate;
			msmsdcc_state_rt_suspend_mmc1 = 0;
			mmc->ops->set_ios(host->mmc, &host->mmc->ios);
		}
	#endif /* CONFIG_IDLECTRL_EMMC_CUST_SH */
...
#endif /* !defined(CONFIG_MMC_SDIO_CUST_SH) && !defined(CONFIG_IDLECTRL_EMMC_CUST_SH) */
	return -EAGAIN;
}
...
static void msmsdcc_pm_idle_resume_lower(struct mmc_host *mmc){
	...
	if (mmc) {
	  	...
	  	if (mmc->card && (mmc->card->type == MMC_TYPE_MMC)) {
			if (msmsdcc_state_rt_suspend_mmc1 != 1) {
				mmc->ios.clock = host->clk_rate;
			}
		} else {
			if(mmc->ios.vdd != 0){
