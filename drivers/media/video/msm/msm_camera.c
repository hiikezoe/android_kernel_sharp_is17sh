...
#if defined(CONFIG_SH_MMAP)
#include "sh_mmap.h"
#endif /* CONFIG_SH_MMAP */
...
#if 1
	if (udata->timeout_ms > timeout) {
		timeout = udata->timeout_ms;
	}

	qcmd_resp = __msm_control(sync,
				  &ctrl_pmsm->ctrl_q,
				  qcmd, msecs_to_jiffies(timeout));
#else
	qcmd_resp = __msm_control(sync,
				  &ctrl_pmsm->ctrl_q,
...
#if (defined(CONFIG_T8EV4) || defined(CONFIG_S5K4EC)) && defined(CONFIG_SHCAM_TOOLS)
	case MSM_CAM_IOCTL_CTRL_SENSOR_COMMAND:
		rc = sh_camera_ioctl(pmsm->sync, argp);
		break;
#endif /* (defined(CONFIG_T8EV4) || defined(CONFIG_S5K4EC)) && defined(CONFIG_SHCAM_TOOLS) */
...
static int __msm_release(struct msm_sync *sync)
{
	...
	if (!sync->opencnt) {
		...
		if (sync->core_powered_on) {
			...
			if(sync->sctrl.s_release)   /* marge */
			sync->sctrl.s_release();

#if defined(CONFIG_T8EV4) || defined(CONFIG_S5K4EC)
			if(sync->sctrl.s_power_ctrl)
				sync->sctrl.s_power_ctrl(sync->pdev, 0);
...
#if defined(CONFIG_T8EV4) || defined(CONFIG_S5K4EC)
			if(sync->sctrl.s_power_ctrl) {
				rc = sync->sctrl.s_power_ctrl(sync->pdev, 1);
				if (!rc) {
					pr_err("%s: setting sensor power & clocks failed: %d\n",
						__func__, rc);
					rc = -EIO;
					goto msm_open_err;
				}
