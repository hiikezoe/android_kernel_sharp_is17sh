...
#define SH_I2C_INVALID_RETRY_SLAVE_ADDRESS	0x30
...
{
	struct qup_i2c_dev *dev = i2c_get_adapdata(client->adapter);

	mutex_lock(&dev->mlock);
}
EXPORT_SYMBOL(sh_qupi2c_mutex_lock);
...
EXPORT_SYMBOL(sh_qupi2c_mutex_free);
#endif /* #if defined( CONFIG_I2C_CUST_SH ) */
...
#if defined( CONFIG_I2C_CUST_SH )
{
	...
	uint16_t SlaveAddress = msgs[0].addr;

	if(SlaveAddress == SH_I2C_INVALID_RETRY_SLAVE_ADDRESS){
		retry_cnt = 0;
	}else{
		retry_cnt = XFER_RETRY;
	}
