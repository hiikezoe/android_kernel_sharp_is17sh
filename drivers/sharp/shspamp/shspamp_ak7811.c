...
/* [stop geiger when using speaker/vibration] <- */
	...
	if(spamp_data){
		...
		shspamp_i2c_write(spamp_data->client_p, 0x06, 0x0d);                 /* [LOUD] HDRM:+2dB, Loud:LD1 */
/* [Protect Off] [start] */
		shspamp_i2c_write(spamp_data->client_p, 0x1f, 0x96);                 /* Step1:Into TestMode */
		shspamp_i2c_write(spamp_data->client_p, 0x1A, 0x02);                 /* Step2:Protect Off */
/* [Protect Off] [end] */
...
int __init shspamp_init(void)
{
	...
	if(rc < 0){
		pr_err("ak7811:alloc_chrdev_region error\n");
		return rc;
	}
	...
	if (IS_ERR(shspamp_class)) {
		...
		pr_err("ak7811:class_create error\n");
	...
	if (IS_ERR(shspamp_device)) {
		...
		pr_err("ak7811:device_create error\n");
	...
	if(rc < 0){
		pr_err("ak7811:cdev_add error\n");
		goto err_via_cdev_add;
	}
	
	rc = shspamp_i2c_init();
	if(rc < 0){
		pr_err("ak7811:shspamp_i2c_init error\n");
		goto err_via_cdev_add;
	}
	
	rc = gpio_request(SHSPAMP_PDN, SHSPAMP_PDN_MSG);
	if(rc < 0){
		pr_err("ak7811:gpio_request error\n");
	...
	if(rc < 0){
		pr_err("ak7811:gpio_tlmm_config error\n");
		goto err_via_gpio_config;
	}
#endif
