...
static ssize_t shtemp_moni_info_store( struct kobject *kobj, struct attribute *attr, const char *buff, size_t len )
{
    ...
    if( !strncmp( buff, "low", strlen("low")) ){
        memset( e_num, 0x00, sizeof(e_num) );
        snprintf( e_num, EVENT_NAME_MAX - 1, "HIGH_TEMP_DETECT=low" );
    }
    else if( !strncmp( buff, "high", strlen("high")) ){
        memset( e_num, 0x00, sizeof(e_num) );
        snprintf( e_num, EVENT_NAME_MAX - 1, "HIGH_TEMP_DETECT=high" );
    }
    else {
        printk( "%s : line %d error\n", __FUNCTION__, __LINE__ );
        return -1;
