/* drivers/sharp/shtps/sy3000/tm2179-001/shtps_rmi_spi.c
 *
 ...
 */
	...
	static int SHTPS_POS_FILTER_WIDTH_FINGER_THRESH = 	13;
	static int SHTPS_POS_FILTER_WIDTH_FINGER_THRESH2 = 	9;
	...
	#define SHTPS_POS_FILTER_WIDTH_FINGER_THRESH	13
	#define SHTPS_POS_FILTER_WIDTH_FINGER_THRESH2	9
	...
	for(page = 0;page < 2;page++){
		...
		for(i = 0xE9;i > 0x0a;i-=sizeof(pdt)){
			...
			case 0x11:
				...
				rc = shtps_rmi_read(ts, ts->map.fn11.ctrlBase + 0x3C, &ts->map.fn11.ctrl.sensor_rx_num, 1);
				if(rc){
					goto err_exit;
				}
				rc = shtps_rmi_read(ts, ts->map.fn11.ctrlBase + 0x3D, &ts->map.fn11.ctrl.sensor_tx_num, 1);
				if(rc){
					goto err_exit;
				}
...
static int shtps_get_tm_rxsize(struct shtps_rmi_spi *ts)
{
	...
	if(ts->map.fn05.enable != 0){
		receive_num = ts->map.fn11.ctrl.sensor_rx_num;
	}else{
		receive_num = ts->map.fn11.ctrl.sensor_rx_num;
	}
...
static int shtps_get_tm_txsize(struct shtps_rmi_spi *ts)
{
	...
	if(ts->map.fn05.enable != 0){
		trans_num   = ts->map.fn11.ctrl.sensor_tx_num;
	}else{
		trans_num   = ts->map.fn11.ctrl.sensor_tx_num;
	}
...
#if defined( SHTPS_SPI_BLOCKREAD_ENABLE )
	shtps_rmi_read(ts, ts->map.fn01.dataBase, buf, readsize + 2 * 5);
