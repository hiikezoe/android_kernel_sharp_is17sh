...
static struct perf_lock dtv_perf_lock;
...
{
	printk("%s:%d !!!tuner_release() start \n", __FILE__, __LINE__);
	mutex_lock(&dtv_lock);
	num--;
	if (num == 0) {
		perf_unlock(&dtv_perf_lock);
		printk(KERN_DEBUG "[perflock] DTV perflock disabled.\n");
	}
	mutex_unlock(&dtv_lock);

	printk("%s:%d !!!tuner_release() end \n", __FILE__, __LINE__);
	return (0);
}
...
static int __init tuner_init(void)
{
	...
	mutex_init(&dtv_lock);
	perf_lock_init(&dtv_perf_lock, PERF_LOCK_768MHz, "DTV");

	printk("%s:%d !!!tuner_init() end \n", __FILE__, __LINE__);
	return 0;
}
...
static int tuner_clk_enable(void)
{
	...
	printk("%s:%d !!!tuner_clk_enable() start \n", __FILE__, __LINE__);
	...
	printk("%s:%d !!!tuner_clk_enable() end \n", __FILE__, __LINE__);
	return 0;
}

static int tuner_clk_disable(void)
{
	printk("%s:%d !!!tuner_clk_disable() start \n", __FILE__, __LINE__);
	clk_disable( gp_clk );
	printk("%s:%d !!!tuner_clk_disable() end \n", __FILE__, __LINE__);
...
static int tuner_vreg_enable(void)
{
	...
	printk("%s:%d !!!tuner_vreg_enable() start \n", __FILE__, __LINE__);
	...
	printk("%s:%d !!!tuner_vreg_enable() end \n", __FILE__, __LINE__);
	return 0;
}
...
static int tuner_vreg_disable(void)
{
	...
	printk("%s:%d !!!tuner_vreg_disable() start \n", __FILE__, __LINE__);
	vreg_gp5 = vreg_get(NULL, "gp5");
	if (IS_ERR(vreg_gp5)) {
		printk(KERN_ERR "%s: vreg_get(%s) failed (%ld)\n",
			__func__, "gp5", PTR_ERR(vreg_gp5));
	...
	printk("%s:%d !!!tuner_vreg_disable() end \n", __FILE__, __LINE__);
	return 0;
}
