...
/* Compile Switch */
#if defined( CONFIG_MACH_DECKARD_AS36 ) || defined( CONFIG_MACH_DECKARD_AS46 )
#define SHPEM_INIT_SHDN_ENABLE
#endif /* CONFIG_MACH_DECKARD_AS36 */
#define SHPEM_SHTERM_ENABLE
#define SHPEM_I2C_MUTEX_ENABLE
//#define SHPEM_SHUTDOWN_ENABLE
...
#define SHPEM_COM_TIMEOUT_READ_SUBLCD	msecs_to_jiffies(200)
...
/* about subLCD */
#define SHPEM_SUBDISP_MAX_WIDTH	(304 / 8)
#define SHPEM_SUBDISP_MAX_HIGHT	56
#define SHPEM_SUBDISP_MAX_SIZE	SHPEM_SUBDISP_MAX_WIDTH * SHPEM_SUBDISP_MAX_HIGHT
...
const shpem_command_t shpem_command[] = 
{
	...
	{"DSCE",										"DSCE",										"EROR"},
	{"DSFPxxxxs",									"DSFP",										"EROR"},
	{"DSIVxs",										"DSIVxr",									"ERORxx"},
	{"DSBSxxs",										"DSBS",										"EROR"},
	{"DSBE",										"DSBE",										"EROR"},
	{"DSPSxs",										"DSPSxr",									"ERORxx"},
};
...
static int shpem_not_support_cmd_check(unsigned char cmd_id)
{
	switch(cmd_id)
	{
		...
		case SHPEM_CMD_DSBE:
		case SHPEM_CMD_DSPS:
#endif /* CONFIG_MACH_DECKARD_AF21 */
			return 0;
		default:
			return -1;
	}
...
static shpem_result_t shpem_com_read(shpem_com_param_t* param)
{
	...
	else if((param->cmd_id == SHPEM_CMD_DSEN) || (param->cmd_id == SHPEM_CMD_DSCS) || (param->cmd_id == SHPEM_CMD_DSFP))
	{
		timeout = wait_event_timeout(shpem_com_wq, (atomic_read(&shpem_com_flag) != 0), SHPEM_COM_TIMEOUT_READ_SUBLCD);
	}
	else
	{
...
static shpem_result_t shpem_ioctl_com(shpem_com_param_t* argp)
{
	...
	shpem_com_param_t param;
	static unsigned char send_buf[10+SHPEM_SUBDISP_MAX_SIZE+1] = {0};
...
#else
	sema_init(&shpem_sem, 1);
	sema_init(&shpem_ioctl_sem, 1);
	sema_init(&shpem_sleep_sem, 1);
	sema_init(&shpem_sensor_sem, 1);
#endif
	atomic_set( &shpem_com_flag, 0 );
...
#else
static long shpem_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
#endif
{
