        ...
        size_t count)
{
    ...
    if (hwdep_driver.set_enable(value) == 0) {
        if (value) {
            ...
            if (hwdep_driver.set_delay(200) == 0)
            {
                data->delay = 200;
            }
        }
        ...
        char *buf)
{
    ...
    if (hwdep_driver.set_enable(1) == 0) {
        ...
        if (measure_normal(&busy, &t, &x, &y1, &y2) >= 0) {
            ...
            yas_cdrv_set_transformatiom_matrix(YAS_TRANSFORMATION[CONFIG_SH_YAS530_POSITION]);
...
#else
            if( (input_abs_get_val(data->input_data, ABS_X) != tmpX) ||
                (input_abs_get_val(data->input_data, ABS_Y) != tmpY) ||
                (input_abs_get_val(data->input_data, ABS_Z) != tmpZ) )
            {
                /*  Magnetic Accuracy Init */
                input_abs_set_val(data->input_data, ABS_STATUS, 99);
            }
#endif
