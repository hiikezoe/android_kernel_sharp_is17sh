...
static int mpu_pm_notifier_callback(struct notifier_block *nb, unsigned long event,
				void *unused)
{
			...
			char __user *buf, size_t count, loff_t *offset)
{
	...
	err = copy_from_user(&local_mpu_pm_event, 
			buf, 
		      ...
		      unsigned int cmd, unsigned long arg)
{
	...
	case MPU_READ_REGISTER:
		{
			...
			if (mldl_cfg->addr == param.address) {
				retval = sensor_i2c_read(this_client->adapter, mldl_cfg->addr, *(unsigned char *)param.data, param.length, (unsigned char *)(param.data + 1));
			} else if (mldl_cfg->pdata->compass.address == param.address) {
				retval = sensor_i2c_read(compass_adapter, mldl_cfg->pdata->compass.address, *(unsigned char *)param.data, param.length, (unsigned char *)(param.data + 1));
			} else if (mldl_cfg->pdata->accel.address == param.address) {
				retval = sensor_i2c_read(accel_adapter, mldl_cfg->pdata->accel.address, *(unsigned char *)param.data, param.length, (unsigned char *)(param.data + 1));
			}
			if (retval) {
				break;
			}
	...
	case MPU_WRITE_REGISTER:
		{
			...
			if (mldl_cfg->addr == param.address) {
				retval = sensor_i2c_write_register(this_client->adapter, mldl_cfg->addr, *(unsigned char *)param.data, *(unsigned char *)(param.data + 1));
			} else if (mldl_cfg->pdata->compass.address == param.address) {
				retval = sensor_i2c_write_register(compass_adapter, mldl_cfg->pdata->compass.address, *(unsigned char *)param.data, *(unsigned char *)(param.data + 1));
			} else if (mldl_cfg->pdata->accel.address == param.address) {
				retval = sensor_i2c_write_register(accel_adapter, mldl_cfg->pdata->accel.address, *(unsigned char *)param.data, *(unsigned char *)(param.data + 1));
			}
		}
		break;
	case MPU_GET_PORTSTATE:
		{
			char gpio_state[4];
			gpio_state[0] = gpio_get_value(20);     /* ACCEL_INT    */
			gpio_state[1] = gpio_get_value(101);    /* GYRO_INT     */
			gpio_state[2] = gpio_get_value(117);    /* COMPASS_DRDY */
			gpio_state[3] = gpio_get_value(181);    /* COMPASS_INT  */
		  ...
		  const struct i2c_device_id *devid)
{
	...
	struct vreg *msdrv_vreg = NULL;

	dev_dbg(&client->adapter->dev, "%s\n", __func__);

    msdrv_vreg = vreg_get(NULL, "gp13");
    vreg_set_level(msdrv_vreg, 3000);
    vreg_enable(msdrv_vreg);

    gpio_request(101,"MPUIRQ");
    ...
    gpio_tlmm_config( GPIO_CFG(181,0, GPIO_CFG_INPUT,GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE );
