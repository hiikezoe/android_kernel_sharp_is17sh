/* drivers/sharp/irda/ir_LR388.c (IrDA SIR/FIR driver module)
 *
...
#define	IRDASYS_HIRDAINTPOL			0x0100
#define	IRDASYS_IRDATESTSEL_LR388J5		0x0400
...
static const uint16	IR_CR36_DELAY = (8);

void __iomem *lr388_hcs1 = NULL;
void __iomem *lr388_hcs2 = NULL;
...
void ir_reg_CR36_write(uint16 cr36_write_val)
{
	IR_WRITE_CR36(cr36_write_val);

	udelay(IR_CR36_DELAY);
}
...
TYPE_IR_REG_RESULT ir_reg_ioremap_nocache(void)
{
	...
	lr388_hcs1 = ioremap_nocache(IRDA_ADDR_HCS1, IRDA_ADDR_HCS1_SIZE);
	if (lr388_hcs1 == NULL) {
		w_ret = IR_REG_RESULT_FAIL;
		MSG_IRREG_FATAL("lr388_hcs1=0x%x\n", (int)lr388_hcs1);
	} else {
		lr388_hcs2 = ioremap_nocache(IRDA_ADDR_HCS2,
							IRDA_ADDR_HCS2_SIZE);
		if (lr388_hcs2 == NULL) {
			w_ret = IR_REG_RESULT_FAIL;
			MSG_IRREG_FATAL("lr388_hcs2=0x%x\n",
							(int)lr388_hcs2);
		}
	}
exit:
...
void ir_reg_irdadrv_fir_set_led(void)
{
	...
	register uint16	w_dmy	= IR_CR15_SD_TERM;
...
void ir_reg_irda_sir_int_finish(void)
{
	uint16	sr36 = 0;

	sr36 = IR_READ_SR36;

	if ((sr36 & IR_SR36_UART_INT_MODE) == IR_SR36_UART_INT_MODE) {
		IR_WRITE_CR38(IR_CR38_SIR_INT_END_SEQ);
	}
}
void ir_reg_irda_sir_int_err_finish(void)
{
	uint16	sr36 = 0;
...
void ir_reg_irda_sir_set_rx_adr_match(void)
{
	...
	ir_reg_CR36_write(IR_CR36_UART_INT_MODE);
	IR_WRITE_CR26(IR_CR26_FIFO_RESET);
...
void ir_reg_irda_sir_set_tx(TYPE_IR_SEND_KIND a_kind)
{
	...
	if (a_kind == IR_SEND_CONTINUE) {
		ir_reg_CR36_write(IR_CR36_TX_END_TIMING2 |
						IR_CR36_UART_INT_MODE);
...
void ir_reg_irda_fir_set_tx(TYPE_IR_SEND_KIND a_kind)
{
	...
	if (a_kind == IR_SEND_CONTINUE) {
		IR_WRITE_CR11(IR_CR11_SEND_CONTINUE);
		ir_reg_CR36_write(IR_CR36_FIR_INT_MODE);
	} else {
		IR_WRITE_CR11(IR_CR_CLEAR);
		ir_reg_CR36_write(IR_CR_CLEAR);
	}
...
TYPE_IR_REG_RESULT ir_reg_irda_carrier_chk(void)
{
	...
	if ((a_sr1 & IR_SR1_CRRIA_RACH) != IR_SR_CLEAR) {
		...
		ir_reg_CR0_reset(IR_CR0_TIMER_RESET);
		w_ret = IR_REG_RESULT_FAIL;
	}
