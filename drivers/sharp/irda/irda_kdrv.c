...
#define IRDA_KERNEL_API_WARNING_KMALLOC		(1)
...
#define	IRDA_MEMALLOC_TRY_COUNT_MAX	(3)
#define	IRDA_MEMALLOC_RETRY_INTERVAL	(50)
...
static int irda_kdrv_check_index(void)
{
	...
	if (irda_mem != NULL) {
		if ((irda_mem->receive.control.wp >= IRDA_MEM_FRAME_MAX) ||
			(irda_mem->receive.control.rp >= IRDA_MEM_FRAME_MAX) ||
			(irda_mem->send.control.wp >= IRDA_MEM_FRAME_MAX) ||
			(irda_mem->send.control.rp >= IRDA_MEM_FRAME_MAX)) {
			ret = IRDA_KERNEL_API_ERROR;
			MSG_IRDRV_ERR("illegal index error\n"
						"receive.control.wp = %hu\n"
						"receive.control.rp = %hu\n"
						"send.control.wp = %hu\n"
						"send.control.rp = %hu\n",
...
static void irda_kdrv_isr_sir_recv_err(void)
{

	MSG_IRDRV_ERR("isr sir receive err\n");

	irda_kdrv_set_mode(IRDA_KDRV_MODE_STOP, IRDA_KDRV_LED_NO_CHG);

	ir_reg_irda_err_reset();

	ir_reg_irda_sir_int_err_finish();

	irda_kdrv_set_mode(IRDA_KDRV_MODE_RECV, IRDA_KDRV_LED_NO_CHG);
}
...
{
	irda_kdrv_set_mode(IRDA_KDRV_MODE_STOP, IRDA_KDRV_LED_CHG);
	irda_kdrv_set_mode(IRDA_KDRV_MODE_RECV, IRDA_KDRV_LED_CHG);
}
...
static int irda_kdrv_memory_alloc(void)
{
	...
	if (IrDA_kmalloc_ptr == NULL) {
		...
		if (ir_reg_ioremap_nocache() == IR_REG_RESULT_SUCCESS) {
			...
			for (tryCnt = 0;
				 tryCnt < IRDA_MEMALLOC_TRY_COUNT_MAX;
				 				tryCnt++) {

				if (tryCnt != 0) {
					msleep(IRDA_MEMALLOC_RETRY_INTERVAL);

					MSG_IRDRV_ERR("kmalloc "
						"retry count:%d\n", tryCnt);
				}

				IrDA_kmalloc_ptr = kmalloc(
						IRDA_MMAP_LENGTH + PAGE_SIZE,
						GFP_KERNEL);
				if (IrDA_kmalloc_ptr != NULL) {
					break;
				}
			...
			if (IrDA_kmalloc_ptr != NULL) {
				IrDA_kdrv_mmap_p =
				(void *)ALIGN(
				(unsigned long)IrDA_kmalloc_ptr,
							PAGE_SIZE);
			} else {
				IrDA_kdrv_mmap_p = NULL;
				ret = IRDA_MEM_ERR_KMALLOC;

				MSG_IRDRV_ERR("Retry results"
...
{
	int ret = IRDA_KERNEL_API_SUCCESS;
	int memory_alloc_ret = IRDA_MEM_OK;
	...
	if (ret == IRDA_KERNEL_API_SUCCESS) {
		memory_alloc_ret = irda_kdrv_memory_alloc();
		if (memory_alloc_ret != IRDA_MEM_OK) {
			MSG_IRDRV_ERR("memory alloc = %d\n",
							memory_alloc_ret);

			IrDA_kdrv.state = IRDA_KDRV_STATUS_CLOSE;

			if (memory_alloc_ret == IRDA_MEM_ERR_KMALLOC) {
				MSG_IRDRV_ERR("WARNING memory alloc\n");
				ret = -ENOMEM;
			} else {
				ret = -ENXIO;
			}
								...
								loff_t *f_pos)
{
	...
	if (IrDA_kdrv.first_send == true) {
		...
		irda_kdrv_set_mode(IRDA_KDRV_MODE_SEND, IRDA_KDRV_LED_CHG);
...
static struct file_operations IrDA_kdrv_fops = {
	...
	.unlocked_ioctl = irda_kdrv_fops_ioctl,
...
static int irda_kdrv_init_func(void)
{
	...
	int gpio_ret = IRDA_KERNEL_API_SUCCESS;

	irda_kdrv_init_control();

	irda_kdrv_init_kernel_func();

	gpio_ret = irda_kdrv_init_gpio();
	if (gpio_ret == IRDA_KERNEL_API_SUCCESS) {
			...
			if (memalloc_ret != IRDA_MEM_OK) {
				...
				if (memalloc_ret == IRDA_MEM_ERR_KMALLOC) {
					ret = IRDA_KERNEL_API_WARNING_KMALLOC;
					MSG_IRDRV_ERR("WARNING memory alloc\n");
				} else {
					ret = -ENXIO;
				}
				...
				struct device_attribute *attr, char *buf)
{
	...
	result = scnprintf(buf, PAGE_SIZE,
					"%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n",
						...
						IRDA_CAPABILITY_MTT,
						IRDA_CAPABILITY_MPI,
						IRDA_CAPABILITY_BUFF,
						IRDA_CAPABILITY_HWBUFF,
						IRDA_CAPABILITY_SWTXBUFF,
						IRDA_CAPABILITY_SWRXBUFF);
...
static ssize_t irda_kdrv_capability_val_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	ssize_t result;

	result = scnprintf(buf, PAGE_SIZE, "%s%s%s%s%s\n",
						IRDA_CAPABILITY_BAUDRATE_VAL,
						IRDA_CAPABILITY_ABOF_VAL,
						IRDA_CAPABILITY_MTT_VAL,
						IRDA_CAPABILITY_MPI_VAL,
						IRDA_CAPABILITY_BUFF_VAL);

	return result;
}

static struct device_attribute irda_kdrv_capability_val_attr = {
	.attr = {
		.name = "capability_val",
		.mode = S_IRUGO,
	},
	.show = irda_kdrv_capability_val_show,
	.store = NULL,
};
						...
						struct platform_device *pdev)
{
	...
	ret = device_create_file(&pdev->dev, &irda_kdrv_capability_val_attr);
	if (ret != IRDA_KERNEL_API_SUCCESS) {
		MSG_IRDRV_FATAL("capability_val attr create error = %d\n", ret);
	}
...
static int irda_kdrv_platform_driver_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &irda_kdrv_capability_val_attr);
