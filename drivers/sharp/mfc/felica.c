...
/* RFS */
#define D_RFS_DEVS		(1)
#define D_RFS_GPIO_NO	(110)
#define D_RFS_DEV_NAME	("felica_rfs")

/* INT */
#define D_INT_DEVS		(1)
#define D_INT_GPIO_NO	(111)
#define D_INT_DEV_NAME	("felica_int")

/* PON */
#define D_PON_DEVS		(1)
#define D_PON_GPIO_NO	(109)
...
static unsigned int rws_sts = D_RWS_RW_ENABLE;
		      ...
		      size_t len, loff_t * ppos)
{
	...
	FELICA_DRV_DBG_LOG("START");
	
	/* length check */
	if ( len < 1 ) {
		FELICA_DRV_ERR_LOG("length check len = %d", len);
		return -EIO;
	}
	
	ret = gpio_get_value(D_RFS_GPIO_NO);
	if (ret < 0) {
		FELICA_DRV_ERR_LOG("gpio_get_value ret = %d", ret);
		return ret;
	}
	...
	if (copy_to_user(buf, on, len)) {
		FELICA_DRV_ERR_LOG("copy_to_user");
		return -EFAULT;
	}
	
	FELICA_DRV_DBG_LOG("END on[0] = %d, len = %d", on[0], len);
...
static int rfs_init(void)
{
	...
	if (sdResult) {
		FELICA_DRV_ERR_LOG("alloc_chrdev_region sdResult = %d", sdResult);
		return sdResult;
	}
	...
	if (sdResult) {
		...
		FELICA_DRV_ERR_LOG("cdev_add sdResult = %d", sdResult);
		return sdResult;
	}
	...
	if (IS_ERR(class_dev)) {
		...
		FELICA_DRV_ERR_LOG("device_create sdResult = %d", sdResult);
		return sdResult;
	}
	
	FELICA_DRV_DBG_LOG("END");
		       ...
		       size_t len, loff_t *ppos)
{
	...
	if (copy_from_user(&on, data, 1)) {
		FELICA_DRV_ERR_LOG("copy_from_user");
		return -EFAULT;
	}
	...
	if (on == SHMFD_PON_STATUS_HIGH)
		...
		FELICA_DRV_ERR_LOG("on = %d", on); 
		return -EFAULT;
	}
	
	/* pon high status check */
	if((on == SHMFD_PON_STATUS_HIGH) && (pon_high_sts == D_PON_HIGH_DISABLE)){
		FELICA_DRV_ERR_LOG("High Sts on = %d, pon_high_sts = %d", on, pon_high_sts); 
		return -EBUSY;
	}
	
	gpio_set_value(D_PON_GPIO_NO , seton );
	
	FELICA_DRV_DBG_LOG("END on = %d, seton = %d", on, seton);
...
static long pon_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	default:
		FELICA_DRV_ERR_LOG("cmd = %d, pon_high_sts = %d", cmd, pon_high_sts);
		return -EINVAL;
	}
	
	FELICA_DRV_DBG_LOG("END cmd = %d, pon_high_sts = %d", cmd, pon_high_sts);
...
void rfs_poll_work_func(struct work_struct *work)
{
	...
	FELICA_DRV_DBG_LOG("read_value = %d old_value = %d", read_value, old_value);
	...
	/* read changed data */
		...
		if (irq_set_irq_type(gpio_to_irq(D_RFS_POLL_GPIO_NO), irqflag))
			FELICA_DRV_ERR_LOG("set_irq_type irqflag = %ld", irqflag);
	}
	...
	FELICA_DRV_DBG_LOG("END read_value = %d, old_value = %d, rfs_d->read_error = %d"
...
static irqreturn_t rfs_poll_irq_handler(int irq, void *dev_id)
{
	...
	FELICA_DRV_DBG_LOG("START irq = %d", irq);
...
unsigned int rfs_poll_poll(struct file *file, poll_table *wait)
{
	...
	FELICA_DRV_DBG_LOG("END mask = %d", mask);
		      ...
		      size_t len, loff_t * ppos)
{
	...
	if (!rfs_d->irq_handler_done) {
		...
		ret = wait_event_interruptible(rfs_d->read_wait,
		                               rfs_d->irq_handler_done == 1);
		if (-ERESTARTSYS == ret) {
			FELICA_DRV_DBG_LOG("wait_event_interruptible ret = %d", ret);
			return -EINTR;
		}
	}
	...
	if (rfs_d->read_error) {
		...
		rfs_d->read_error = 0;
		FELICA_DRV_ERR_LOG("rfs_d->read_error = %d", rfs_d->read_error);
		return -EIO;
	}
	...
	FELICA_DRV_DBG_LOG("END len = %d, on[0] = %d", len, on[0]);
...
static int rfs_poll_open(struct inode *inode, struct file *file)
{
	...
	if (rfs_d->open_flag) {
		FELICA_DRV_ERR_LOG("only one time");
		return -EBUSY;
	}
	rfs_d->open_flag = 1;
	...
	if (request_irq(gpio_to_irq(D_RFS_POLL_GPIO_NO),
		...
		FELICA_DRV_ERR_LOG("request_irq irqflag = %ld", irqflag);
	...
	if (enable_irq_wake(gpio_to_irq(D_RFS_POLL_GPIO_NO))){
		...
		FELICA_DRV_ERR_LOG("enable_irq_wake");
...
static int rfs_poll_release(struct inode *inode, struct file *file)
{
	...
	if (disable_irq_wake(gpio_to_irq(D_RFS_POLL_GPIO_NO)))
		FELICA_DRV_ERR_LOG("disable_irq_wake");
...
void int_poll_work_func(struct work_struct *work)
{
	...
	FELICA_DRV_DBG_LOG("END read_value = %d, old_value = %d, int_d->read_error = %d"
		      ...
		      size_t len, loff_t * ppos)
{
	...
	if (int_d->read_error) {
		...
		int_d->read_error = 0;
		FELICA_DRV_ERR_LOG("int_d->read_error = %d", int_d->read_error);
		return -EIO;
	}
		      ...
		      size_t len, loff_t * ppos)
{
	...
	FELICA_DRV_DBG_LOG("END on = %d, len = %d", on[0], len);
		       ...
		       size_t len, loff_t *ppos)
{
	...
	if (on == SHMFD_RWS_RW_ENABLE)
		rws_sts = D_RWS_RW_ENABLE;
	else if (on == SHMFD_RWS_RW_DISABLE)
		rws_sts = D_RWS_RW_DISABLE;
	else {
		FELICA_DRV_ERR_LOG("on = %d", on);
		return -EFAULT;
	}
	
	FELICA_DRV_DBG_LOG("END on = %d rws_sts = %d", on, rws_sts);
...
int felica_ppc_read_E2PROM(unsigned char item_id, unsigned char* ppc_data_ptr)
{
	...
	FELICA_DRV_DBG_LOG("START item_id = %d", item_id);
	
	/* NULL check */
	if (ppc_data_ptr == NULL) {
		FELICA_DRV_ERR_LOG("ppc_data_ptr NULL");
	...
	if ((item_id < FELICA_PPC_E2PROM_ITEM_ID_MIN) ||
		...
		FELICA_DRV_ERR_LOG("item_id = %d", item_id);
	...
	if (i2c_ret < TRUE) {
		FELICA_DRV_ERR_LOG("read_status_register");
	...
	if (ppc_check_por_state == FELICA_PPC_STATUS_POR) {
		FELICA_DRV_ERR_LOG("PPC_state_of_POR");
	...
	if (get_status_register != FELICA_PPC_STATUS_E2PROM_ACCESSMODE) {
		...
		if (i2c_ret < TRUE) {
			FELICA_DRV_ERR_LOG("change_accessmode");
...
int felica_ppc_read_register(unsigned char item_id, unsigned char* ppc_data_ptr)
{
	...
	case FELICA_PPC_SET_PORT_DATA_COMMAND:					/* set port				*/
		*ppc_data_ptr = get_status_register & FELICA_PPC_STATUS_OUTPUT_MASK;
		FELICA_DRV_DBG_LOG("END SET_PORT_DATA_COMMAND");
		return TRUE;
	case FELICA_PPC_SET_SLAVEADDRESS_COMMAND:				/* set slave address	*/
		FELICA_DRV_DBG_LOG("SET_SLAVEADDRESS_COMMAND");
		break;
	default:
		return FALSE;
	}
...
int felica_ppc_write_E2PROM(unsigned char item_id, unsigned char set_ppc_data)
{
	...
	FELICA_DRV_DBG_LOG("START item_id = %d, set_ppc_data = %d", item_id, set_ppc_data);
	...
	if ((item_id == FELICA_PPC_SET_PORT_DATA_COMMAND) ||
		...
		FELICA_DRV_DBG_LOG("item_id = 0x06 or 0x07 SET_WAIT");
	} else if (item_id == FELICA_PPC_CHANGE_ACCESSMODE_COMMAND) {
		FELICA_DRV_DBG_LOG("item_id = 0x04 SET_NO_WAIT");	/* write register 		*/
	} else {
		FELICA_DRV_ERR_LOG("item_id = %d", item_id);
		return FALSE;
	}
	...
	if (check_data_ret == FALSE) {
		FELICA_DRV_ERR_LOG("set_ppc_data = %d", set_ppc_data);
	...
	if (i2c_ret < TRUE) {
		...
		FELICA_DRV_ERR_LOG("E2PROM_write_protect");
	...
	if ((item_id == FELICA_PPC_CHANGE_ACCESSMODE_COMMAND) &&
		...
		i2c_ret = mfd_LD_ppc_read_status_register_process(&get_status_register);
		if (i2c_ret < TRUE) {
			FELICA_DRV_ERR_LOG("read_status_register(registaer)");
		...
		/* case : registermode				*/
		if(get_status_register == FELICA_PPC_STATUS_E2PROM_ACCESSMODE) {
			FELICA_DRV_ERR_LOG("accsessmode(registaer)");
...
int felica_ppc_reverse_output(void)
{
	...
	if (i2c_ret < TRUE) {
		FELICA_DRV_ERR_LOG("reverse_output");
...
int felica_ppc_write_protect(int write_protect)
{
	...
	if (i2c_ret < TRUE) {
		FELICA_DRV_ERR_LOG("write_protect");
...
static int mfd_LD_ppc_E2PROM_write_protect_process(int write_protect)
{
	...
	FELICA_DRV_DBG_LOG("START write_protect = %d", write_protect);
	...
	for (count = 0; count < FELICA_PPC_I2C_TRY_COUNT; count++) {
		...
		FELICA_DRV_DBG_LOG("i2c_retry count = %d", count);
	}
	FELICA_DRV_DBG_LOG("END i2c_ret = %d", i2c_ret);
...
static int mfd_LD_ppc_read_status_register_process(unsigned char* ppc_data_ptr)
{
	...
	for (count = 0; count < FELICA_PPC_I2C_TRY_COUNT; count++) {
		...
		if (i2c_ret > FALSE) {
			...
			FELICA_DRV_DBG_LOG("i2c_retry_count = %d ", count);
		}
	}
...
static int mfd_LD_ppc_read_process(unsigned char cmd, unsigned char* read_data_ptr)
{
	...
	FELICA_DRV_DBG_LOG("START cmd = %d", cmd);
...
static int mfd_LD_ppc_change_accessmode_process(int accessmode)
{
	...
	FELICA_DRV_DBG_LOG("START accessmode = %d", accessmode);
...
static int mfd_LD_ppc_write_process(unsigned char cmd, unsigned char set_data, int wait_flg)
{
	...
	FELICA_DRV_DBG_LOG("START cmd = %d, set_data = %d, wait_flg = %d", cmd, set_data, wait_flg);
	...
	for (count = 0; count < FELICA_PPC_I2C_TRY_COUNT; count++) {
		...
		if (i2c_ret > FALSE) {
			...
			if (i2c_ret > FALSE) {
				switch (cmd) {
				case FELICA_PPC_CHANGE_ACCESSMODE_COMMAND:		/* change access mode	*/
					FELICA_DRV_DBG_LOG("CHANGE_ACCESSMODE_COMMAND");
					get_ppc_data &= FELICA_PPC_STATUS_ACCESSMODE_MASK;
					get_ppc_data >>= 1;
					break;
				case FELICA_PPC_SET_PORT_DATA_COMMAND:			/* set port				*/
					FELICA_DRV_DBG_LOG("SET_PORT_DATA_COMMAND");
...
static int mfd_LD_ppc_check_set_data(unsigned char cmd, unsigned char set_data)
{
	...
	FELICA_DRV_DBG_LOG("START cmd = %d, set_data = %d" ,cmd, set_data);
	...
	if(set_data <= ppc_set_max_data) {
		...
		FELICA_DRV_ERR_LOG("set_data = %d, ppc_set_max_data = %d", set_data, ppc_set_max_data);
		return FALSE;
	}
}
...
static int mfd_PD_ppc_change_accessmode(int accessmode)
{
	...
	if (i2c_ret > FALSE) {
		...
		FELICA_DRV_ERR_LOG("mfd_PD_ppc_i2c_write i2c_ret = %d", i2c_ret);
		return FALSE;
	}
}
...
static int mfd_PD_ppc_read(unsigned char cmd, unsigned char* read_data_ptr)
{
	...
	if (read_data_ptr == NULL) {
		FELICA_DRV_ERR_LOG("read_data_ptr NULL");
	...
	if (extract_data_ret > FALSE) {
		...
		FELICA_DRV_ERR_LOG("extract_read_data extract_data_ret = %d",extract_data_ret);
		return FALSE;
	}
}
...
static int mfd_PD_ppc_write(unsigned char cmd, unsigned char set_data, int wait_flg)
{
	...
	FELICA_DRV_DBG_LOG("END write_ppc_ret = %d", write_ppc_ret);
...
static int mfd_PD_ppc_E2PROM_write_protect(int write_protect)
{
	...
	if (i2c_ret > FALSE) {
		...
		FELICA_DRV_ERR_LOG("i2c_ret = %d", i2c_ret);
...
static int mfd_PD_ppc_reload(void)
{
	...
	FELICA_DRV_DBG_LOG("END reload_ret = %d", reload_ret);
...
static int mfd_PD_ppc_reverse_output(void)
{
	...
	FELICA_DRV_DBG_LOG("END reverse_ret = %d", reverse_ret);
...
static int mfd_PD_ppc_extract_read_data(unsigned char cmd, unsigned char* read_data_ptr)
{
	...
	FELICA_DRV_DBG_LOG("END read_data = %d", *read_data_ptr);
				...
				size_t count, loff_t *pos)
{
	...
	/* check NULL_ptr		*/
	if (buf == NULL) {
		FELICA_DRV_ERR_LOG("buf_ptr_NULL");
		return -EFAULT;
	}
	...
	FELICA_DRV_DBG_LOG("END lock_status = %d ,sizeof(lock_status) = %d", lock_status, sizeof(lock_status));
				...
				size_t count, loff_t *pos)
{
	...
	default:
		mutex_unlock(&lock);
		FELICA_DRV_ERR_LOG("lock_status = %d", lock_status);
		return -EINVAL;
	}
	...
	if (lock_retern == TRUE) {
		FELICA_DRV_DBG_LOG("END sizeof(lock_request) = %d", sizeof(lock_request));
		return (sizeof(lock_request));
	} else {
		FELICA_DRV_ERR_LOG("lock_retern = %d", lock_retern);
		return -EIO;
	}
}
...
static long cen_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case (FELICA_PPC_READ_E2PROM):
		FELICA_DRV_DBG_LOG("START FELICA_PPC_READ_E2PROM");
		if (copy_from_user(&felica_lock_status, (struct mfd_cen __user *)arg, sizeof(felica_lock_status))) {
			FELICA_DRV_ERR_LOG("copy_from_user read E2PROM");
			return -EFAULT;
		}
		
		item_id			= felica_lock_status.item_id;
		if (felica_ppc_read_E2PROM(item_id, &read_data_ptr) == FALSE) {
			FELICA_DRV_ERR_LOG("felica_ppc_read_E2PROM");
			return -EIO;
		}
		
		FELICA_DRV_DBG_LOG("read_data = 0x%02x read E2PROM", read_data_ptr);
		...
		if (copy_to_user((struct mfd_cen __user *)arg, &felica_lock_status, sizeof(felica_lock_status))) {
			FELICA_DRV_ERR_LOG("copy_to_user read_E2PROM");
			return -EFAULT;
		}
		break;
	...
	case (FELICA_PPC_READ_REGISTER):
		FELICA_DRV_DBG_LOG("START FELICA_PPC_READ_REGISTER");
		if (copy_from_user(&felica_lock_status, (struct mfd_cen __user *)arg, sizeof(felica_lock_status))) {
			FELICA_DRV_ERR_LOG("copy_from_user read register");
			return -EFAULT;
		}
		...
		if (felica_ppc_read_register(item_id, &read_data_ptr) == FALSE) {
			FELICA_DRV_ERR_LOG("felica_ppc_read_register");
			return -EIO;
		}
		
		FELICA_DRV_DBG_LOG("read_data = 0x%02x read_register", read_data_ptr);
		
		felica_lock_status.data = read_data_ptr;
		
		if (copy_to_user((struct mfd_cen __user *)arg, &felica_lock_status, sizeof(felica_lock_status))) {
			FELICA_DRV_ERR_LOG("copy_to_user read_register");
			return -EFAULT;
		}
		break;
	...
	case (FELICA_PPC_WRITE_E2PROM):	
		...
		FELICA_DRV_DBG_LOG("START FELICA_PPC_WRITE_E2PROM");
		if (copy_from_user(&felica_lock_status, (struct mfd_cen __user *)arg, sizeof(felica_lock_status))) {
			FELICA_DRV_ERR_LOG("copy_from_user write_E2PROM");
			return -EFAULT;
		}
		...
		if (felica_ppc_write_E2PROM(item_id, write_data_ppc) == FALSE) {
			FELICA_DRV_ERR_LOG("felica_ppc_write_E2PROM");
			return -EIO;
		}
		break;
	...
	case (FELICA_PPC_WRITE_REGISTER):
		...
		FELICA_DRV_DBG_LOG("START FELICA_PPC_WRITE_REGISTER");
		if (copy_from_user(&felica_lock_status, (struct mfd_cen __user *)arg, sizeof(felica_lock_status))) {
			FELICA_DRV_ERR_LOG("copy_from_user write_register");
			return -EFAULT;
		}
		...
		if (felica_ppc_write_register(item_id, write_data_ppc) == FALSE) {
			FELICA_DRV_ERR_LOG("felica_ppc_write_register");
			return -EIO;
		}
	...
	case (FELICA_PPC_RELOAD):
		...
		FELICA_DRV_DBG_LOG("START FELICA_PPC_RELOAD");
		
		if (felica_ppc_reload() == FALSE) {
			FELICA_DRV_ERR_LOG("felica_ppc_reload");
			return -EIO;
		}
	...
	case (FELICA_PPC_REVERSE_OUTPUT):
		...
		FELICA_DRV_DBG_LOG("START FELICA_PPC_REVERSE_OUTPUT");
		
		if (felica_ppc_reverse_output() == FALSE) {
			FELICA_DRV_ERR_LOG("felica_ppc_reverse_output");
			return -EIO;
		}
	...
	case (FELICA_PPC_WRITE_PROTECT):
		...
		FELICA_DRV_DBG_LOG("START FELICA_PPC_WRITE_PROTECT");
		
		if (copy_from_user(&felica_lock_status, (struct mfd_cen __user *)arg, sizeof(felica_lock_status))) {
			FELICA_DRV_ERR_LOG("copy_from_user write_protect");
			return -EFAULT;
		}
		...
		if (write_protect < FELICA_PPC_WRITE_DISABLE ||
			...
			FELICA_DRV_ERR_LOG("write_protect = %d", write_protect);
			return -EINVAL;
		}
		
		if (felica_ppc_write_protect(write_protect) == FALSE) {
			FELICA_DRV_ERR_LOG("felica_ppc_write_protect");
			return -EIO;
		}
		break;
...
static int cen_probe(struct i2c_client *client, const struct i2c_device_id * devid)
{
	...
	if (alloc_ret) {
		FELICA_DRV_ERR_LOG("alloc_chrdev_region");
		return alloc_ret;
	}
