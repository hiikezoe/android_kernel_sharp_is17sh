...
static int sh_pm8xxx_upl_init( void )
{
	struct pm8xxx_upl_device *upl_dev;
	int rc = 0;

	upl_dev = pm8xxx_upl_request();

	rc = pm8xxx_upl_write_truthtable(upl_dev, 0x000a);

    if(rc){
		printk(KERN_ERR "%s: write truthtable FAIL: 0x%x \n", __func__ , rc );
		return rc;
    }
    rc = pm8xxx_upl_config( upl_dev, 
                              PM8XXX_UPL_MOD_ENABLE_MASK | PM8XXX_UPL_OUT_DTEST_MASK | PM8XXX_UPL_IN_A_MASK | PM8XXX_UPL_IN_B_MASK | PM8XXX_UPL_IN_C_MASK | PM8XXX_UPL_IN_D_MASK, 
                              PM8XXX_UPL_MOD_ENABLE      | PM8XXX_UPL_OUT_GPIO_ONLY  | PM8XXX_UPL_IN_A_GPIO | PM8XXX_UPL_IN_B_GPIO | PM8XXX_UPL_IN_C_GPIO | PM8XXX_UPL_IN_D_GPIO  );
    if(rc){
...
#ifdef CONFIG_SHPMIC_UPL
static struct sh_pm8xxx_upl_platform_data sh_pm8xxx_upl_data = {
	.init		= sh_pm8xxx_upl_init,
};
#endif /* CONFIG_SHPMIC_UPL */
...
static struct mfd_cell upl_cell __devinitdata = {
	...
	.id		= -1,
#ifdef CONFIG_SHPMIC_UPL
	.platform_data	= &sh_pm8xxx_upl_data,
	.pdata_size	= sizeof(sh_pm8xxx_upl_data),
#endif /* CONFIG_SHPMIC_UPL */
};
