...
static void dbs_refresh_callback(struct work_struct *unused)
{
	...
	if (policy->cur < this_dbs_info->freq_table[3].frequency) {
		policy->cur = this_dbs_info->freq_table[3].frequency;
		__cpufreq_driver_target(policy, this_dbs_info->freq_table[3].frequency,
					CPUFREQ_RELATION_L);
